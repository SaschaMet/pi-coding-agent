...
describe("subagent chain execution", () => {
    ...
    it("ignores parent container CLI flags", async () => {
        const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "pi-subagent-ignore-container-flags-"));
        fs.mkdirSync(path.join(tmp, ".pi", "agents"), { recursive: true });
        fs.mkdirSync(path.join(tmp, "node_modules", ".bin"), { recursive: true });
        fs.writeFileSync(path.join(tmp, "node_modules", ".bin", "pi"), "#!/bin/sh\nexit 0\n", "utf-8");
        fs.writeFileSync(
        ...
        try {
            spawnMock.mockImplementation((_command: string, args: string[]) => {
                ...
                expect(args).not.toContain("--sandbox-persist");
            ...
            const result = await tool!.execute(
                "call-subagent-ignore-container-flags",
                {
                    agent: "planner",
                    task: "plan",
                    agentScope: "project",
    ...
    it("uses node from PATH for fallback subagents and ignores parent container flags", async () => {
        ...
        try {
            spawnMock.mockImplementation((command: string, args: string[]) => {
                ...
                expect(args[0]).toBe(path.join(tmp, "wrapper.js"));
                expect(args).not.toContain("--container");
                expect(args).not.toContain("--container-net");
