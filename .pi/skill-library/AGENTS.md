...
The whole point: 13 skills here add **zero** tokens to a session that does not use them. Move a skill to `../skills/` only when you want it triggered without asking.
...
5. **Add a row to the index table above.** A skill absent from the index is invisible.
...
3. Delete its row from the index table above.
