...
* **NO fake-perfect numbers.** Avoid `99.99%`, `50%`, `1234567`. Use organic, messy data (`+1 (312) 847-1928`, `14:32`, `Mar 4`, `$1,284.50`).
  **This is not licence to invent statistics** — see 4.9, which bans AI-invented spec precision like `92%` or `4.1×`. The two rules split on *what kind of number it is*: incidental detail (phone numbers, dates, times, order totals, row counts in a mock table) should look messy rather than suspiciously round, because round incidental data reads as placeholder. A **claim about the product or business** — uptime, speed multiples, customer counts, accuracy — must come from the brief or be labeled mock. Messy detail, yes; invented evidence, no.
...
* **Work in the existing stack. Do not migrate frameworks or styling libraries.** Before importing any new library, read the project's dependency file first. If the project uses Tailwind, check the version (v3 vs v4) before touching config — a v4 `@theme` block does nothing in a v3 project, silently, and a v3 `tailwind.config` is not read by v4. If the project has no framework, use vanilla CSS.
* **Keep changes reviewable.** Small targeted improvements over big rewrites; do not break existing functionality.
