...
- Code comments are minimal: no doc comments on functions, methods, or classes by default. At most one short comment per non-obvious block, and only for a `why` (constraint, workaround, invariant). Never paraphrase code into prose.
- No lint or doc-lint rule is added or kept that requires documentation comments.
...
5. Implement only the selected standard:
   ...
   - keep code comments minimal: no doc comments by default; at most one short `why` comment per non-obvious block; never paraphrase code into prose; if the repo already has a doc-comment convention, preserve it but do not expand it
   - do not add or tighten linter/doc-linter rules that require doc comments; if such a rule already exists in the repo, list it in the gap analysis and ask before weakening or removing it
