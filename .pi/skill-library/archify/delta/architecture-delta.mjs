...
function sortedObjects(values) {
  return [...values].sort((left, right) =>
    codepointOrder(canonical(left), canonical(right)),
  );
}
...
function normalizeRepository(repository) {
  ...
  return {
    ...
    ...(repository.link_mode === undefined
      ? {}
      : { link_mode: repository.link_mode }),
  };
}
...
function normalizeComponent(component) {
  return {
    ...
    ...(Array.isArray(component.sources)
      ? { sources: sortedObjects(component.sources) }
      : {}),
  };
}
...
export function canonicalArchitecture(diagram) {
  ...
  return {
    ...
    boundaries: sortedBy(
      (diagram.boundaries || []).map(normalizeBoundary),
      boundaryKey,
    ),
    connections: sortedBy(
      diagram.connections || [],
...
function requireComparableShape(diagram, side) {
  if (diagram?.schema_version !== 1) {
    fail(
      ...
      `${side} must use schema_version 1.`,
      { side, path: "/schema_version", actual: diagram?.schema_version },
    );
  }
...
function stableIndex(
  ...
  collection,
  side,
  missingCode = "delta/stable-id-required",
) {
  const index = new Map();
  const missing = [];
  const duplicates = [];
  ...
  if (missing.length) {
    fail(
      missingCode,
      `${side} ${collection} require authored stable ids for comparison.`,
      {
        side,
  ...
  if (duplicates.length) {
    fail(
      "delta/duplicate-stable-id",
      `${side} ${collection} contain duplicate ids.`,
      {
        side,
        collection,
...
function boundaryIndex(boundaries, side) {
  ...
  if (ambiguous.length) {
    fail(
      "delta/boundary-key-ambiguous",
      `${side} boundary kind + label keys must be unique.`,
      {
        side,
...
function fieldChanges(before, after, groups) {
  ...
  for (const [classification, fields] of Object.entries(groups)) {
    const changed = fields.filter(
      (field) =>
        !equal(normalizedField(before, field), normalizedField(after, field)),
    );
  ...
  return {
    ...
    changedFields: sorted(changedFields),
  };
}
...
function statusFor(classifications, kind) {
  if (
    classifications.some((value) =>
      ["topology", "semantic", "scope"].includes(value),
    )
  )
    return "changed";
...
function compareEntities(baseIndex, headIndex, kind, groups, describe) {
  ...
  for (const id of sorted(
    new Set([...baseIndex.keys(), ...headIndex.keys()]),
  )) {
    ...
    if (!base)
      ...
      if (status !== "same")
        changes.push({ ...describe(id, base, head), status, ...fields });
    } else
      changes.push({
...
function summaryFor(changes, shape) {
  ...
  for (const change of changes) {
    const key = change.status.replace(/-([a-z])/g, (_all, letter) =>
      letter.toUpperCase(),
    );
...
export function compareArchitecture(base, head, evidence = {}) {
  ...
  if (!shared.length) {
    fail(
      ...
      {
        supportedFixes: [
          "preserve at least one authored component id across snapshots",
        ],
      },
    );
  }
  ...
  const identity = (repository) =>
    parseRepositoryRemote(repository.url, { authored: true })?.identity ||
    repository.url;
  if (
    baseRepository &&
    headRepository &&
    identity(baseRepository) !== identity(headRepository)
  ) {
    fail(
      "delta/repository-mismatch",
      "The snapshots name different repositories.",
      {
  ...
  const connections = compareEntities(
    ...
    (id, before, after) => ({
      ...
      ...(before
        ...
        : {}),
      ...(after
        ? { head: { from: after.from, to: after.to, label: after.label || "" } }
        : {}),
    }),
  );
  ...
  return {
    ...
    base: {
      ...
      ...(evidence.baseSemanticSha256
        ? { semanticSha256: evidence.baseSemanticSha256 }
        : {}),
      ...(Number.isInteger(evidence.baseBytes)
        ? { bytes: evidence.baseBytes }
        : {}),
      ...(baseRepository?.revision
    ...
    head: {
      ...
      ...(evidence.headSemanticSha256
        ? { semanticSha256: evidence.headSemanticSha256 }
        : {}),
      ...(Number.isInteger(evidence.headBytes)
        ? { bytes: evidence.headBytes }
        : {}),
      ...(headRepository?.revision
        ? { revision: headRepository.revision }
        : {}),
    },
    summary: {
      ...
      connections: summaryFor(connections, [
        "added",
        "changed",
        "removed",
        "rerouted",
      ]),
      boundaries: summaryFor(boundaries, [
        "added",
        "changed",
        "removed",
        "geometryChanged",
...
export function extractArchitectureSvg(html) {
  const match = html.match(
    /<svg viewBox="0 0 [^"]+" role="img"[\s\S]*?<\/svg>/,
  );
  if (!match)
    fail(
      "delta/svg-missing",
      "A validated Architecture artifact did not contain its primary SVG.",
    );
  return match[0];
}
...
export function extractArtifactCss(html) {
  ...
  if (!match)
    fail(
      "delta/css-missing",
      "A validated Architecture artifact did not contain its stylesheet.",
    );
  return match[1];
}
...
function boundaryChangeMap(changes) {
  return new Map(
    changes.map((change) => [`${change.kind}:${esc(change.label)}`, change]),
  );
}
...
export function annotateArchitectureSideSvg(svg, receipt, side) {
  ...
  result = transformNodeGroups(result, (group, id) => {
    ...
    if (
      ...
      return group;
    const tagged = group.replace(/^<g[^>]+>/, (tag) =>
      addState(tag, change, side),
    );
    return addNodeMarker(tagged, change?.status);
  });
  result = result.replace(
    /<(?:path|g)\s+[^>]*\bdata-edge-id="([^"]+)"[^>]*>/g,
    (tag, id) => addState(tag, edges.get(id), side),
  );
...
function elementById(svg, kind, id) {
  ...
  const path =
    svg.match(
      new RegExp(`<path\\s+[^>]*\\bdata-edge-id="${safe}"[^>]*/>`),
    )?.[0] || "";
  const label =
    svg.match(
      new RegExp(`<g\\s+[^>]*\\bdata-edge-id="${safe}"[^>]*>[\\s\\S]*?<\\/g>`),
    )?.[0] || "";
...
function boundaryMarkupParts(markup) {
  ...
  for (const element of boundaryElements(markup))
    parts[element.part] = element.markup;
  return parts;
}
...
function boundarySymbolMarkup(markup, state) {
  const symbol = markerFor(state);
  const frame = markup.match(
    /<rect[^>]*\bx="([\d.-]+)"\s+y="([\d.-]+)"\s+width="([\d.-]+)"/,
  );
...
export function buildDeltaSvg(baseSvg, headSvg, receipt) {
  ...
  for (const change of edges.values()) {
    if (
      ...
      change.classifications.includes("topology")
    ) {
  ...
  for (const change of boundaries.values()) {
    ...
    if (change.status === "removed") {
      ...
      change.status === "geometry-changed"
    ) {
      const phantom = forceBoundaryState(
        boundaryMarkupByKey(baseSvg, renderedKey),
        "moved-from",
  ...
  for (const change of boundaries.values()) {
    if (!["added", "changed", "geometry-changed"].includes(change.status))
      continue;
    const renderedKey = `${change.kind}:${esc(change.label)}`;
    boundaryMarkers.push(
      boundarySymbolMarkup(
        boundaryMarkupByKey(delta, renderedKey),
        change.status,
      ),
    );
  }
...
function reviewPrimaryStates(row) {
  ...
  if (
    row.kindKey === "boundary" &&
    ["changed", "geometry-changed"].includes(row.status)
  )
...
function reviewTargetSignature(tags) {
  return tags
    .map(({ name, tag }) => {
      ...
      const classifications =
        tag.match(/\bdata-delta-classifications="([^"]*)"/)?.[1] || "";
...
function expectedReviewTargetSignature(row) {
  ...
  if (row.kindKey === "component") {
    for (const state of reviewPrimaryStates(row))
      descriptors.push(`g:${state}:${classifications}`);
    ...
    for (const form of forms) {
      descriptors.push(
        `path:${form.state}:${classifications}`,
        `text:${form.marker}:`,
      );
...
export function renderArchitectureDeltaHtml({
  ...
  const proof =
    receipt.proofLevel === "revision-pinned"
...
export function validateArchitectureDeltaHtml(html, receipt) {
  ...
  if (
    !["base", "delta", "head"].every(
      (id) =>
        (
          html.match(
            new RegExp(`<section class="canvas" data-view="${id}"`, "g"),
          ) || []
        ).length === 1,
    )
  )
  ...
  if (
    (
      html.match(
        /class="snapshot-frame" title="Before architecture explorer"/g,
      ) || []
    ).length !== 1 ||
    (
      html.match(
        /class="snapshot-frame" title="After architecture explorer"/g,
      ) || []
    ).length !== 1
  ) {
    failures.push(
      "Before and After must preserve one complete architecture explorer each",
    );
  }
  if (
    !svgBalanced ||
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const rowMatches = [
      ...html.matchAll(
        new RegExp(
          `<button class="change-row"[^>]*data-change-key="${safeKey}"[^>]*>`,
          "g",
        ),
      ),
    ].map((match) => match[0]);
    if (rowMatches.length !== 1)
      failures.push(`expected exactly one change row ${row.key}`);
    ...
    const storedSignature = rowMatches[0]?.match(
      /\bdata-change-target-signature="([^"]*)"/,
    )?.[1];
    if (
      !signature ||
    ...
    if (
      rowMatches[0]?.match(/\bdata-change-index="([^"]+)"/)?.[1] !==
      String(index)
    )
    ...
    const states = primary
      ...
      .sort();
    if (
      JSON.stringify(states) !== JSON.stringify(reviewPrimaryStates(row).sort())
    )
  ...
  if (/\b(?:NaN|Infinity)\b/.test(html))
    failures.push("contains non-finite output");
  if (receipt.completeness !== "complete")
    failures.push("receipt is not complete");
  if (failures.length)
    fail(
