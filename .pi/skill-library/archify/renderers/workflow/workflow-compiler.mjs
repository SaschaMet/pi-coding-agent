...
function readableGroupBounds(workflow, group, colXs) {
  if (
    !Number.isInteger(group.fromCol) ||
    !Number.isInteger(group.toCol) ||
    group.fromCol < 0 ||
    group.fromCol > group.toCol ||
    group.toCol >= colXs.length
  ) {
  ...
  let left =
    group.fromCol === group.toCol && width > naturalWidth
      ? start
      : (start + end - width) / 2;
...
function createReadableLayout(workflow, layoutFeedback = {}) {
  ...
  for (let leftIndex = 0; leftIndex < nodes.length; leftIndex += 1) {
    for (
      ...
      if (leftNode.lane !== rightNode.lane || leftNode.col === rightNode.col)
        continue;
      ...
      constraints.push({
        ...
        to: toNode.col,
        minimum:
          authoredNodeWidth(fromNode) / 2 + 8 + authoredNodeWidth(toNode) / 2,
        contributors: [
  ...
  for (const edge of asArray(workflow.edges)) {
    ...
    if (
      ...
      !["auto", "straight"].includes(edge.route || "auto")
    )
      continue;
    if ((Number(fromNode.yOffset) || 0) !== (Number(toNode.yOffset) || 0))
      continue;
    const earlier = fromNode.col < toNode.col ? fromNode : toNode;
    const later = earlier === fromNode ? toNode : fromNode;
    const labeledDirectClearance =
    ...
    const canUseAutomaticLabelChannel =
      edge.label &&
      !edge.labelAt &&
      ...
      edge.channelX === undefined &&
      edge.channelY === undefined;
    const preferLabelChannel =
      canUseAutomaticLabelChannel &&
    ...
    constraints.push({
      ...
      to: later.col,
      minimum:
        authoredNodeWidth(earlier) / 2 + 28 + authoredNodeWidth(later) / 2,
      contributors: [
    ...
    if (!preferLabelChannel && labeledDirectClearance > 28) {
      const labelConstraintMinimum =
        authoredNodeWidth(earlier) / 2 +
  ...
  for (const phase of asArray(workflow.phases)) {
    if (
      !Number.isInteger(phase.fromCol) ||
      !Number.isInteger(phase.toCol) ||
      phase.fromCol < 0 ||
      phase.fromCol > phase.toCol ||
      phase.toCol >= columnCount
    )
      continue;
  ...
  for (let iteration = 0; iteration < maxLayoutIterations; iteration += 1) {
    colXs = Array.from(
      { length: columnCount },
      (_, col) => columnStart + col * baselinePitch,
    );
    ...
    for (let to = 1; to < columnCount; to += 1) {
      for (const constraint of orderedConstraints) {
        ...
        if (candidate > colXs[to] + 0.0001) {
          ...
          Math.abs(candidate - colXs[to]) <= 0.0001 &&
          candidate > columnStart + to * baselinePitch + 0.0001
        ) {
          for (const contributor of candidateProvenance)
            colProvenance[to].add(contributor);
        }
      }
    }
  ...
  if (leftShift) {
    ...
    for (const node of firstRankNodes) {
      if (Math.abs(authoredNodeWidth(node) / 2 - firstExtent) > 0.0001)
        continue;
      for (const provenance of colProvenance)
        provenance.add(nodeWidthContributor(node));
    }
  }

  const unpinnedTopEndpointIds = new Set();
  for (const edge of asArray(workflow.edges)) {
    const preservesHorizontalPins =
      Array.isArray(edge.via) || edge.channelX !== undefined;
  ...
  const laneOrder = new Map(
    asArray(workflow.lanes).map((lane, index) => [lane.id, index]),
  );
  ...
  for (const nodeId of unpinnedTopEndpointIds) {
    ...
    const laneHeaderRight =
      40 + 14 + textUnits(`${prefix} / ${lane.label}`) * 6.2;
    ...
    if (requiredShift > laneHeaderShift + 0.0001) {
      ...
      requiredShift > 0 &&
      Math.abs(requiredShift - laneHeaderShift) <= 0.0001
    ) {
      laneHeaderShiftContributors.add(`lane ${lane.id} label width`);
    }
  }
  if (laneHeaderShift > 0) {
    for (let col = 0; col < colXs.length; col += 1)
      colXs[col] += laneHeaderShift;
    for (const provenance of colProvenance) {
      for (const contributor of laneHeaderShiftContributors)
        provenance.add(contributor);
    }
  }

  let measuredContentLeftShift = asArray(workflow.edges).reduce(
    (maximum, edge) => {
  ...
  for (const group of asArray(workflow.groups)) {
    ...
    measuredContentLeftShift = Math.max(
      measuredContentLeftShift,
      44 - bounds.x,
    );
  }
  if (measuredContentLeftShift > 0) {
    for (let col = 0; col < colXs.length; col += 1)
      colXs[col] += measuredContentLeftShift;
  }

  let rightmost = colXs.at(-1) + 50;
  let rightmostContributors = new Set(colProvenance.at(-1));
  for (const node of nodes) {
    if (!Number.isInteger(node.col) || node.col < 0 || node.col >= columnCount)
      continue;
    ...
    if (nodeRight > rightmost + 0.0001) {
      ...
      for (const contributor of nodeContributors)
        rightmostContributors.add(contributor);
    }
  }
  for (const group of asArray(workflow.groups)) {
    if (!Number.isInteger(group.fromCol) || !Number.isInteger(group.toCol))
    ...
    if (groupRight > rightmost + 0.0001) {
      ...
      for (const contributor of groupContributors)
        rightmostContributors.add(contributor);
    }
  }
  const widestLaneLabel = asArray(workflow.lanes).reduce(
    (widest, lane, index) => {
      const width =
        textUnits(`${String(index + 1).padStart(2, "0")} / ${lane.label}`) *
          6.2 +
        30;
  ...
  const laneW = Math.max(640, rightmostLaneWidth, Math.ceil(laneLabelWidth));
  if (laneW > 640) {
    if (rightmostLaneWidth === laneW) {
      for (const contributor of rightmostContributors)
        widthContributors.add(contributor);
    }
    if (Math.ceil(laneLabelWidth) === laneW && widestLaneLabel.lane) {
      widthContributors.add(
        `lane ${widestLaneLabel.lane.id || widestLaneLabel.lane.label} label width`,
      );
    }
  }
  ...
  for (const group of asArray(workflow.groups)) {
    groupsByLane.set(group.lane, [
      ...(groupsByLane.get(group.lane) || []),
      group,
    ]);
  }
  const groupLaneReserves = asArray(workflow.lanes).map((lane) => {
    ...
    for (const group of groupsByLane.get(lane.id) || []) {
      ...
      for (const node of nodes) {
        ...
        const topOffset =
          (baseContentH - authoredNodeHeight(node)) / 2 +
  ...
  const laneHeights = groupLaneReserves.map(
    ({ header, footer }) => laneH + header + footer,
  );
  ...
  for (const [index, reserve] of groupHeaderHeights.entries()) {
    ...
    heightContributors.add(
      `lane ${lane.id || lane.label} group label clearance ${reserve}px`,
    );
  }
  for (const [index, reserve] of groupFooterHeights.entries()) {
    ...
    const lane = asArray(workflow.lanes)[index];
    heightContributors.add(
      `lane ${lane.id || lane.label} group frame containment ${reserve}px`,
    );
  }
  if (laneH > 104) {
    for (const contributor of verticalExtentContributors)
...
function canonicalReadableWorkflow(workflow) {
  ...
  const nodes = [...asArray(workflow.nodes)].sort(
    (left, right) =>
  ...
  const edges = [...asArray(workflow.edges)].sort(
    (left, right) =>
      stableCompare(left.id, right.id) ||
  ...
  const groups =
    workflow.groups === undefined
      ? undefined
      : [...asArray(workflow.groups)].sort(
          (left, right) =>
            (laneOrder.get(left.lane) ?? Number.MAX_SAFE_INTEGER) -
              (laneOrder.get(right.lane) ?? Number.MAX_SAFE_INTEGER) ||
            left.fromCol - right.fromCol ||
...
function semanticContractDiagnostics(workflow) {
  ...
  const referencedNodes = [
    ...asArray(checks.allowedRoots).map((id, index) => ({
      id,
      path: `/semanticChecks/allowedRoots/${index}`,
    })),
    ...asArray(checks.allowedTerminals).map((id, index) => ({
      id,
      path: `/semanticChecks/allowedTerminals/${index}`,
    })),
    ...asArray(checks.requiredEdges).flatMap((relation, index) => [
      {
        id: relation.from,
        path: `/semanticChecks/requiredEdges/${index}/from`,
      },
    ...
    ...asArray(checks.requiredPaths).flatMap((relation, index) => [
      {
        id: relation.from,
        path: `/semanticChecks/requiredPaths/${index}/from`,
      },
  ...
  for (const { id, path } of referencedNodes) {
    ...
    diagnostics.push(
      diagnostic(
        ...
        { knownNodes: [...nodeIds] },
        [
          `replace "${id}" with an existing node id`,
          "add the missing node before compiling",
        ],
      ),
    );
  }
    ...
    for (const [node, count] of incoming) {
      ...
      diagnostics.push(
        diagnostic(
          ...
          [
            `add the missing incoming edge to "${node}"`,
            `declare "${node}" in semanticChecks.allowedRoots if it is an intentional source`,
          ],
        ),
      );
    }
    ...
    for (const [node, count] of outgoing) {
      ...
      diagnostics.push(
        diagnostic(
          ...
          [
            `add the missing outgoing edge from "${node}"`,
            `declare "${node}" in semanticChecks.allowedTerminals if it is an intentional sink`,
          ],
        ),
      );
    }
  ...
  const authoredEdges = new Set(
    asArray(workflow.edges).map((edge) => `${edge.from}\u0000${edge.to}`),
  );
    ...
    diagnostics.push(
      diagnostic(
        ...
        `Workflow semantic contract requires edge "${relation.from}" -> "${relation.to}", but no authored edge matches it.`,
        {
          from: relation.from,
          to: relation.to,
          path: `/semanticChecks/requiredEdges/${index}`,
        },
    ...
    diagnostics.push(
      diagnostic(
        ...
        `Workflow semantic contract requires a directed path from "${relation.from}" to "${relation.to}", but none exists.`,
        {
          from: relation.from,
          to: relation.to,
          path: `/semanticChecks/requiredPaths/${index}`,
        },
        {
          reachableNodes: [
            ...new Set([
...
function compileWorkflowInternal({
    ...
    const diagnostics = [
      {
        ...
        severity: "error",
        message:
          "compileWorkflow requires one parsed workflow document object.",
  ...
  const resolvedQualityProfile =
    qualityProfile || inputWorkflow.meta?.quality_profile;
  const authoredQualityProfile = inputWorkflow.meta?.quality_profile;
  const qualityResolvedWorkflow =
    resolvedQualityProfile &&
    resolvedQualityProfile !== inputWorkflow.meta?.quality_profile
      ? {
          ...inputWorkflow,
          meta: {
            ...inputWorkflow.meta,
  ...
  const sourceIndexes = {
    lanes: new Map(
      asArray(qualityResolvedWorkflow.lanes).map((lane, index) => [
        lane,
        index,
      ]),
    ),
    nodes: new Map(
      asArray(qualityResolvedWorkflow.nodes).map((node, index) => [
        node,
        index,
      ]),
    ),
    edges: new Map(
      asArray(qualityResolvedWorkflow.edges).map((edge, index) => [
        edge,
        index,
      ]),
    ),
  ...
  const LEGEND_CATALOG = [
    ...
    kind,
    label: i18nText(workflow.meta.locale, `legend.workflow.${kind}`),
  }));
  const presentLegendKinds = new Set(
    asArray(workflow.nodes).map((node) => node.type),
  );
  ...
  const minimumCanvasWidth =
    workflow.schema_version === 2
      ? Math.max(
          ...
          oneRowLegendFootprint.minWidth + 40,
        )
      : layout.defaultViewBoxWidth;
  ...
  const legendExtraHeight =
    workflow.schema_version === 2 ? packedLegendFootprint.extraHeight : 0;
  ...
  const laneLabels = new Map(
    asArray(workflow.lanes).map((lane) => [lane.id, lane.label]),
  );
  ...
  function nodeContext(node) {
    const group = asArray(workflow.groups).find(
      (candidate) =>
        ...
        node.col >= candidate.fromCol &&
        node.col <= candidate.toCol,
    );
    ...
    return (
      [laneLabels.get(node.lane), group?.label, phase?.label]
        .filter(Boolean)
        .join(" › ") || i18nText(workflow.meta.locale, "node.context.workflow")
    );
  }

  function laneHeight(idOrIndex) {
    const index =
      typeof idOrIndex === "number" ? idOrIndex : laneIndex.get(idOrIndex);
  ...
  function laneTop(id) {
    ...
    const precedingHeight = asArray(workflow.lanes)
      .slice(0, index)
      .reduce(
        (total, _lane, lanePosition) => total + laneHeight(lanePosition),
        0,
      );
  ...
  function lastLaneBottom() {
    return (
      layout.laneY +
      asArray(workflow.lanes).reduce(
        (total, _lane, index) => total + laneHeight(index),
        0,
      ) +
  ...
  function workflowLegendLayout(obstacles = []) {
    return {
      ...
      baselineY: legendY(),
      width:
        workflow.schema_version === 2 ? legendPackingWidth : viewBox[0] - 40,
      fontSize: 7,
      itemGap: 7,
  ...
  function workflowLegendRects() {
    ...
    const measured = measureLegend(
      workflowLegendEntries,
      workflowLegendLayout(),
    );
    if (!measured) return [];
    return [
  ...
  function measureNode(node) {
    ...
    const contentH =
      laneHeight(node.lane) -
      layout.laneTitleH -
      groupHeaderH -
      laneGroupFooterH(node.lane);
    const y =
      laneTop(node.lane) +
      layout.laneTitleH +
      groupHeaderH +
  ...
  const nodes = new Map(
    asArray(workflow.nodes).map((node) => [node.id, measureNode(node)]),
  );
  ...
  function workflowCompositionFrames() {
    ...
    for (const [index, group] of asArray(workflow.groups).entries()) {
      ...
      frames.push({
        ...
        height:
          workflow.schema_version === 2
            ? laneHeight(group.lane) -
              layout.laneTitleH -
              GROUP_FRAME_TOP_INSET -
              GROUP_FRAME_BOTTOM_INSET
  ...
  function workflowSceneLabelObstacles() {
    ...
    for (const phase of asArray(workflow.phases)) {
      if (
        ...
        !Number.isInteger(phase.toCol) ||
        phase.fromCol < 0 ||
        phase.toCol >= layout.colXs.length ||
        phase.fromCol > phase.toCol
      )
        continue;
    ...
    for (const group of asArray(workflow.groups)) {
      if (
        !laneIndex.has(group.lane) ||
        !Number.isInteger(group.fromCol) ||
        !Number.isInteger(group.toCol) ||
        group.fromCol < 0 ||
        group.toCol >= layout.colXs.length ||
        group.fromCol > group.toCol
      )
        continue;
      const span = groupSpan(group);
      const frameY =
        laneTop(group.lane) + layout.laneTitleH + GROUP_FRAME_TOP_INSET;
  ...
  const mainPathSteps = new Map(
    asArray(workflow.mainPath).map((id, index) => [id, index]),
  );
  const edgeSteps = new Map(
    asArray(workflow.edges).map((edge, index) => {
      ...
      const mainStep =
        Number.isInteger(fromStep) && toStep === fromStep + 1 ? fromStep : null;
  ...
  function verifiedLegacyAlternative(edge, from, to, requiredClearance) {
    ...
    const candidates = layout.colXs
      .map((center, col) => ({ center, col }))
      .filter(({ col }) => col !== to.col)
      .sort(
        (a, b) =>
          Math.abs(a.col - to.col) - Math.abs(b.col - to.col) || a.col - b.col,
      );
    for (const candidate of candidates) {
      const candidateRect = {
        ...
        col: candidate.col,
        cx: candidate.center,
        x: candidate.center - to.width / 2,
      };
      if (occupied.some((node) => rectsOverlap(candidateRect, node, 8)))
        continue;
  ...
  function readableMigrationProvidesCapacity(from, to, requiredClearance) {
    ...
    const centerDistance = Math.abs(
      readable.colXs[to.col] - readable.colXs[from.col],
    );
    if (centerDistance - from.width / 2 - to.width / 2 < requiredClearance)
      return false;
    if (!discoverFixes) return false;
    ...
    return withDiagnosticRecordingSuppressed(() => {
      ...
      const requiredViewBox =
        ...
        compiled.diagnostics.every(
          ({ code }) => code === "workflow/viewbox-capacity",
        )
          ? compiled.diagnostics.find(({ evidence }) =>
              Array.isArray(evidence?.requiredViewBox),
            )?.evidence.requiredViewBox
          : null;
      if (
        !compiled.ok &&
        Array.isArray(candidate.meta?.viewBox) &&
        requiredViewBox
      ) {
  ...
  function verifiedReducedWidths(from, to, requiredClearance) {
    ...
    const labelsFit = candidates.every(
      (node, index) =>
        textUnits(node.label) * 6.8 <= widths[index] + 6 &&
        (!node.sublabel ||
          minimumNodeTextWidth(node.sublabel, nodeTextFit.sublabelMinimum) <=
            availableNodeTextWidth(widths[index])) &&
        (!node.tag ||
          minimumNodeTextWidth(node.tag, nodeTextFit.tagMinimum) <=
            availableNodeTextWidth(widths[index])),
    );
    if (!labelsFit) return null;
    const serializedWidths = widths.map(
      (width) => Math.floor((width + 1e-9) * 100) / 100,
    );
    const signedClearance =
      Math.abs(to.cx - from.cx) -
      serializedWidths[0] / 2 -
      serializedWidths[1] / 2;
    if (signedClearance + 0.0001 < requiredClearance) return null;
    const accepted = acceptsFix((document) => {
      document.nodes.find((node) => node.id === from.id).width =
        serializedWidths[0];
      document.nodes.find((node) => node.id === to.id).width =
        serializedWidths[1];
    });
  ...
  function enforceLegacyColumnCapacity() {
    ...
    for (const edge of workflow.edges) {
      ...
      if (!from || !to || from.lane !== to.lane || from.col === to.col)
        continue;
      if (!verticalIntervalsOverlap(from, to, 8)) continue;
      const centerDistance = Math.abs(to.cx - from.cx);
      const actualSignedClearance =
        centerDistance - from.width / 2 - to.width / 2;
      const direct =
        !edge.via &&
        ["auto", "straight"].includes(edge.route || "auto") &&
      ...
      const alternative = verifiedLegacyAlternative(
        edge,
        from,
        to,
        requiredDirectClearance,
      );
      const reducedWidths = verifiedReducedWidths(
        from,
        to,
        requiredDirectClearance,
      );
      const capacity =
        actualSignedClearance < 0
      ...
      if (
        readableMigrationProvidesCapacity(from, to, requiredDirectClearance)
      ) {
        supportedFixes.push("migrate this workflow to schema_version 2");
      }
      if (alternative !== null)
        supportedFixes.push(
          `move node "${to.id}" to verified free column ${alternative}`,
        );
      if (reducedWidths) {
        supportedFixes.push(
          `set node widths "${from.id}"=${Math.round(reducedWidths[0] * 100) / 100}px and "${to.id}"=${Math.round(reducedWidths[1] * 100) / 100}px`,
        );
      }
  ...
  function verifiedEdgeFix(edge, message, mutator) {
    ...
    const accepted = acceptsFix((document) =>
      mutator(document.edges[edgeIndex], document),
    );
  ...
  function authoredPinEvidence(edge, field) {
    ...
    return {
      ...
      ...(Number.isInteger(authoredEdgeIndex)
        ? { path: `/edges/${authoredEdgeIndex}/${field}` }
        : {}),
      value,
    };
  }
  ...
  function verifiedPinRemovalAlternatives(edge, fields, reason) {
    ...
    const uniqueFields = [
      ...new Set(fields.filter((field) => edge[field] !== undefined)),
    ];
    for (let size = 1; size <= uniqueFields.length; size += 1) {
      ...
      return {
        removalSets,
        supportedFixes: removalSets.map(
          (fieldSet) =>
  ...
  function verifiedPinReferenceAlternatives(candidateRefs, reason) {
    ...
    const fallbackPins = refs.map(({ edge, field }) =>
      authoredPinEvidence(edge, field),
    );
    if (!discoverFixes) {
      return {
        removalSets: [],
        conflictingRefs: refs,
        conflictingPins: fallbackPins,
        repairs: [],
        supportedFixes: [],
      };
    }
    ...
    for (let size = 1; size <= refs.length; size += 1) {
      const removalSets = combinations(refs, size).filter((removalSet) =>
        acceptsFix((document) => {
          for (const { edgeIndex, field } of removalSet)
            delete document.edges[edgeIndex][field];
        }),
      );
      if (!removalSets.length) continue;
      ...
      for (const removalSet of removalSets) {
        for (const { edge, field } of removalSet) {
          ...
          conflictingRefs.push({
            edge,
            edgeIndex: workflow.edges.indexOf(edge),
            field,
          });
  ...
  function verifiedRoutePairPinAlternatives(leftEdge, rightEdge, reason) {
    const refs = [leftEdge, rightEdge].flatMap((edge) => {
      const edgeIndex = workflow.edges.indexOf(edge);
      return authoredRouteAssertionFields(edge).map((field) => ({
        edge,
        edgeIndex,
        field,
      }));
  ...
  function verifiedRepairsWithLabelNudges(alternatives) {
    return alternatives.repairs.flatMap(({ removalSet, message }) => {
      if (removalSet.length !== 1 || removalSet[0].field !== "labelAt")
        return [message];
  ...
  function throwExplicitPinConflict(
    edge,
    invariant,
    evidence,
    supportedFixes = [],
    ...
    const authoredEdgeIndex = sourceIndexes.edges.get(edge);
    const pinPath =
      asArray(evidence?.conflictingPins).length === 1 &&
      Number.isInteger(authoredEdgeIndex) &&
      onlyPin?.field
  ...
  function verifiedRouteGeometryPinAlternatives(
    ...
    const edgeIndex = workflow.edges.indexOf(edge);
    return verifiedPinReferenceAlternatives(
      authoredRouteAssertionFields(edge).map((field) => ({
        edge,
        edgeIndex,
        field,
      })),
  ...
  function properOrthogonalIntersection(
    ...
    if (
      ...
      rightOrientation === "diagonal"
    )
      return null;
    const horizontalStart =
      leftOrientation === "horizontal" ? leftStart : rightStart;
    const horizontalEnd = leftOrientation === "horizontal" ? leftEnd : rightEnd;
    const verticalStart =
      leftOrientation === "vertical" ? leftStart : rightStart;
    ...
    const insideHorizontal =
      point[0] > Math.min(horizontalStart[0], horizontalEnd[0]) + epsilon &&
      point[0] < Math.max(horizontalStart[0], horizontalEnd[0]) - epsilon;
    const insideVertical =
      point[1] > Math.min(verticalStart[1], verticalEnd[1]) + epsilon &&
  ...
  function throwReadableLabelRoutePinConflict(hit, routePoints = null) {
    ...
    const alternatives = verifiedLabelRoutePinAlternatives(
      labelEdge,
      routeEdge,
    );
    const actualRoutePoints =
      routePoints ||
      pathCache.get(routeEdge)?.points ||
      pathFor(routeEdge).points;
    const diagnosticEdge =
      alternatives.conflictingRefs[0]?.edge ||
      (labelPinned ? labelEdge : routeEdge);
    throwExplicitPinConflict(
      diagnosticEdge,
      "explicit label-route clearance",
      {
  ...
  function classifyFailedAutomaticCandidatePins(edge, rawCandidates) {
    ...
    for (const { points } of rawCandidates) {
      ...
      const candidateLabel = candidateRect
        ...
        : null;
      if (candidateLabel) {
        const priorLabel = priorLabels.find(
          (otherLabel) =>
            rectsOverlap(candidateLabel, otherLabel, -2) &&
            (Array.isArray(edge.labelAt) ||
              Array.isArray(otherLabel.relation?.labelAt)),
        );
        if (priorLabel)
          throwReadableLabelLabelPinConflict(candidateLabel, priorLabel);
        ...
        const labelRouteHit = collectLabelRouteClearance({
          ...
          (hit) =>
            Array.isArray(edge.labelAt) ||
            hasAbsoluteRoutePins(hit.otherRelation),
        );
        if (labelRouteHit) {
          const collidedRoute = priorRoutes.find(
            ({ relation }) => relation === labelRouteHit.otherRelation,
          );
          throwReadableLabelRoutePinConflict(
            labelRouteHit,
            collidedRoute?.points,
          );
        }
      }
  ...
  function validateReadablePairwisePinConflicts() {
    const labels = workflow.edges
      .map((edge, relationIndex) => labelRectFor(edge, relationIndex))
      .filter(Boolean);
    const routedRelations = workflow.edges
      .map((edge, relationIndex) =>
    ...
    const labelRouteHit = collectLabelRouteClearance({
      ...
      (hit) =>
        Array.isArray(hit.labelRelation?.labelAt) ||
        hasAuthoredRouteAssertions(hit.otherRelation),
    );
    ...
    for (
      let leftIndex = 0;
      leftIndex < routedRelations.length;
      leftIndex += 1
    ) {
      const left = routedRelations[leftIndex];
      for (
        let rightIndex = leftIndex + 1;
        rightIndex < routedRelations.length;
        rightIndex += 1
      ) {
        ...
        for (const leftSegment of leftAnalysis) {
          for (const rightSegment of rightAnalysis) {
            ...
            if (!point) continue;
            const leftSourceIndex = sourceSegmentIndexAtPoint(
              leftSegment,
              point,
            );
            const rightSourceIndex = sourceSegmentIndexAtPoint(
              rightSegment,
              point,
            );
            const leftSource = {
            ...
            throwExplicitPinConflict(
              ...
              "explicit route-route crossing",
              {
                ...
                routeSegments: [
                  {
                    edge: left.relation.id ?? null,
                    from: [...leftSegment.start],
                    to: [...leftSegment.end],
                  },
                  {
                    edge: right.relation.id ?? null,
                    from: [...rightSegment.start],
                    to: [...rightSegment.end],
                  },
                ],
                sourceRouteSegments: [
    ...
    if (corridorHit) {
      ...
      const diagnosticEdge = leftPinned
        ? corridorHit.left.relation
        : corridorHit.right.relation;
      throwExplicitPinConflict(
        diagnosticEdge,
  ...
  function presentChannelPins(edge) {
    return ["channelX", "channelY"].filter(
      (field) => edge[field] !== undefined,
    );
  }
  ...
  function validateReadableRouteControls(edge) {
    ...
    const conflictingPins = channelPins.filter(
      (field) => !allowedPins.has(field),
    );
  ...
  function corridorTopologyMatches(points, axis, coordinate) {
    ...
    const actualPattern = routeSegments(collapsed).map(
      ({ orientation }) => orientation,
    );
    const expectedPattern = routeSegments(expected).map(
      ({ orientation }) => orientation,
    );
    return (
      actualPattern.length === expectedPattern.length &&
      actualPattern.every(
        (orientation, index) => orientation === expectedPattern[index],
      ) &&
  ...
  function routeMatchesPresetFamily(preset, points, from, to) {
    ...
    if (preset === "drop") {
      ...
      if (collapsed.length === 2 && segments[0]?.orientation === "vertical")
        return true;
  ...
  function validateReadablePinnedGeometry() {
    ...
    for (const edge of workflow.edges) {
      ...
      const negativeViaIndex = asArray(edge.via).findIndex(
        ([x, y]) => x < 0 || y < 0,
      );
      const negativeRoutePin =
        negativeViaIndex >= 0
          ? {
              field: "via",
          ...
          : edge.channelX < 0
            ? {
                field: "channelX",
                path: `/edges/${edgeIndex}/channelX`,
                value: edge.channelX,
              }
            : edge.channelY < 0
              ? {
                  field: "channelY",
                  path: `/edges/${edgeIndex}/channelY`,
                  value: edge.channelY,
                }
              : null;
      if (negativeRoutePin) {
      ...
      if (hasPinnedRoute) {
        const invalidPointIndex = points.findIndex(
          (point) =>
            !Array.isArray(point) ||
            point.length !== 2 ||
            !isFinitePoint(...point),
        ...
        for (
          ...
          if (lengthPx + 0.0001 < minimumPx) {
            ...
            throwExplicitPinConflict(
              edge,
              endpoint
                ? "8px endpoint stub clearance"
                : "16px interior turn clearance",
              {
                conflictingPins: alternatives.conflictingPins,
                segmentIndex,
                position:
        ...
        if (Array.isArray(edge.via)) {
          const missingChannelPins = presentChannelPins(edge).filter(
            (field) => !routeContainsChannelPin(points, field, edge[field]),
          );
        ...
        if (
          ...
          const alternatives = verifiedPinReferenceAlternatives(
            [
              ...
              ...presentRouteGeometryFields(edge).map((field) => ({
                edge,
                edgeIndex: authoredEdgeIndex,
                field,
              })),
            ],
            "and keep the remaining verified route assertions",
        ...
        if (!routeHonorsEndpointSides(points, fromSide, toSide)) {
          const mismatchedSideFields = [
            ...(edge.fromSide &&
            edge.fromSide !== "auto" &&
            !endpointSideIsHonored(points, fromSide, "source")
              ? ["fromSide"]
              : []),
            ...(edge.toSide &&
            edge.toSide !== "auto" &&
            !endpointSideIsHonored(points, toSide, "target")
              ? ["toSide"]
              : []),
          ];
          ...
          throwExplicitPinConflict(
            edge,
            "perpendicular endpoint-side direction",
            {
              conflictingPins: conflictPinsFromRemovalSets(
                edge,
                alternatives.removalSets,
        ...
        const legendObstacle = workflowLegendRects().find((rect) =>
          points
            .slice(0, -1)
            .some((point, index) =>
              segmentIntersectsRect(
                { start: point, end: points[index + 1] },
                rect,
              ),
            ),
        );
        ...
        if (compositionObstacle) {
          ...
          throwExplicitPinConflict(
            edge,
            "lane/phase/group label clearance",
            {
              conflictingPins: alternatives.conflictingPins,
              points: points.map((point) => [...point]),
              compositionObstacle,
        ...
        if (frameRun) {
          ...
          throwExplicitPinConflict(
            edge,
            "structural-frame border clearance",
            {
              conflictingPins: alternatives.conflictingPins,
              points: points.map((point) => [...point]),
      ...
      if (edge.labelAt) {
        ...
        const obstacle =
          rect &&
          [...nodes.values()].find((node) => rectsOverlap(rect, node, -2));
        if (obstacle) {
          throwExplicitPinConflict(
            edge,
            "edge-label node clearance",
            {
        ...
        const legendObstacle =
          rect &&
          workflowLegendRects().find((legendRect) =>
            rectsOverlap(rect, legendRect),
          );
        if (legendObstacle) {
          throwExplicitPinConflict(
            edge,
            "edge-label legend clearance",
            {
              conflictingPins: [authoredPinEvidence(edge, "labelAt")],
              labelAt: [...edge.labelAt],
              labelRect: {
        ...
        const compositionObstacle =
          rect &&
          workflowSceneLabelObstacles().find((candidate) =>
            rectsOverlap(rect, candidate),
          );
        if (compositionObstacle) {
          throwExplicitPinConflict(
            edge,
            "edge-label lane/phase/group clearance",
            {
              conflictingPins: [authoredPinEvidence(edge, "labelAt")],
              labelAt: [...edge.labelAt],
              labelRect: {
  ...
  function validateWorkflow() {
    ...
    for (const node of nodes.values()) {
      ...
      if (node.x < layout.laneX || node.x + node.width > laneRight) {
        problems.push(
          `Node "${node.id}" exceeds the horizontal bounds of lane "${node.lane}".`,
        );
      }
      if (
        node.y < contentTop ||
        node.y + node.height > top + laneHeight(node.lane)
      ) {
        problems.push(
    ...
    for (const phase of asArray(workflow.phases)) {
      if (!Number.isInteger(phase.fromCol) || !Number.isInteger(phase.toCol)) {
        problems.push(
          `Phase "${phase.id}" must use integer fromCol/toCol values.`,
        );
        continue;
      }
      if (
    ...
    for (const group of asArray(workflow.groups)) {
      ...
      if (!Number.isInteger(group.fromCol) || !Number.isInteger(group.toCol)) {
        problems.push(
          `Group "${group.id}" must use integer fromCol/toCol values.`,
        );
        continue;
      }
      if (
    ...
    for (const edge of workflow.edges) {
      if (!nodes.has(edge.from))
        problems.push(
          `Edge "${edge.label || edge.from}" references unknown source "${edge.from}".`,
        );
      if (!nodes.has(edge.to))
        problems.push(
          `Edge "${edge.label || edge.to}" references unknown target "${edge.to}".`,
        );
      if (nodes.has(edge.from) && nodes.has(edge.to)) {
        ...
        if (routed.points.length === 2) {
          ...
          if (segmentLength < 28) {
            problems.push(
              `Edge "${edge.from}" -> "${edge.to}" is too short (${Math.round(segmentLength)}px; minimum 28px) — move the nodes farther apart or use a verified orthogonal route with readable clearance.`,
            );
          }
        }
      }
    ...
    if (Array.isArray(workflow.mainPath)) {
      ...
      for (let i = 0; i < workflow.mainPath.length - 1; i += 1) {
        ...
        if (!from || !to) continue;
        const linked = workflow.edges.some(
    ...
    for (let i = 0; i < labelRects.length; i += 1) {
      for (let j = i + 1; j < labelRects.length; j += 1) {
        if (rectsOverlap(labelRects[i], labelRects[j], -2)) {
          problems.push(
            `Labels "${labelRects[i].label}" and "${labelRects[j].label}" overlap — adjust labelDx/labelDy/labelSegment or route one relationship through a separate corridor.\n${suggestLabelPairFix(labelRects[i], labelRects[j])}`,
          );
        }
      }
    }
  ...
  function validateReadableInputsBeforeRouting() {
    ...
    const fail = (diagnostic) =>
      throwDiagnosticError(diagnostic.message, [diagnostic]);
    ...
    const authoredLanes = [...workflow.lanes].sort(
      (left, right) =>
        sourceIndexes.lanes.get(left) - sourceIndexes.lanes.get(right),
    );
    const authoredNodes = [...workflow.nodes].sort(
      (left, right) =>
        sourceIndexes.nodes.get(left) - sourceIndexes.nodes.get(right),
    );
    const authoredEdges = [...workflow.edges].sort(
      (left, right) =>
    ...
    for (const lane of authoredLanes) {
      ...
      if (firstLaneIndex.has(lane.id)) {
        ...
        const replacement = unusedId(
          lane.id,
          new Set(workflow.lanes.map(({ id }) => id)),
        );
    ...
    for (const node of authoredNodes) {
      ...
      if (firstNodeIndex.has(node.id)) {
        ...
        const replacement = unusedId(
          node.id,
          new Set(workflow.nodes.map(({ id }) => id)),
        );
    ...
    const nodeSourceIndexes = new Map(
      authoredNodes.map((node) => [node.id, sourceIndexes.nodes.get(node)]),
    );
    ...
    for (const authoredNode of authoredNodes) {
      ...
      if (!laneIds.has(node.lane)) {
        ...
        const canonicalNodeIndex = workflow.nodes.findIndex(
      ...
      if (
        ...
        fail({
          ...
          subject: {
            diagramType: "workflow",
            node: node.id,
            path: `/nodes/${nodeIndex}/col`,
          },
          evidence: {
            actualColumn: node.col,
            minimumColumn: 0,
            maximumColumn: layout.colXs.length - 1,
          },
          supportedFixes,
        });
      }
      ...
      for (let left = 0; left < laneNodes.length; left += 1) {
        for (let right = left + 1; right < laneNodes.length; right += 1) {
          if (rectsOverlap(laneNodes[left], laneNodes[right], 8)) {
            ...
            const canonicalNodeIndex = workflow.nodes.findIndex(
              (candidate) => candidate.id === rightNode.id,
            );
            const supportedFixes = layout.colXs.flatMap((_x, col) => {
  ...
  function groupSpan(group) {
    ...
    return spanForCols(group.fromCol, group.toCol, 50, 0);
  }
  ...
  function firstRouteNodeCollision(edge, points) {
    ...
    for (const node of nodes.values()) {
      ...
      for (
        ...
        if (endpointRole === "target-endpoint" && segmentIndex === lastSegment)
          continue;
        const clearancePx = endpointRole === "unrelated" ? 2 : 0;
        const from = points[segmentIndex];
        const to = points[segmentIndex + 1];
        if (
          segmentIntersectsRect({ start: from, end: to }, node, clearancePx)
        ) {
          return {
  ...
  function oneBendCrossLaneVia(edge, start, end, fromSide, toSide) {
    ...
    const segmentsAreReadable = points
      .slice(0, -1)
      .every(
        (point, index) =>
          Math.hypot(
    ...
    if (!segmentsAreReadable || !routeClearsUnrelatedNodes(edge, points))
      return null;
  ...
  function legacyAutomaticOneBendSides(edge, from, to) {
    ...
    return (
      candidates.find(({ fromSide, toSide }) => {
  ...
  function readableAutomaticSides(edge, from, to) {
    ...
    const authoredFrom =
      edge.fromSide && edge.fromSide !== "auto" ? edge.fromSide : null;
    const authoredTo =
      edge.toSide && edge.toSide !== "auto" ? edge.toSide : null;
    ...
    for (const [pairOrdinal, candidate] of sidePairs.entries()) {
      ...
      candidates.push(
        ...planned.candidates.map((route) => ({ ...route, ...candidate })),
      );
    }
  ...
  function routeClearsEndpointNodes(points, from, to) {
    ...
    for (let index = 0; index <= lastSegment; index += 1) {
      ...
      if (index < lastSegment && segmentIntersectsRect(segment, to))
        return false;
    }
    return true;
  }
  ...
  function routeFitsCanvasOrigin(edge, points) {
    return routeExtentCoordinates(edge, points).every(
      ([x, y]) => x >= 0 && y >= 0,
    );
  }

  function readableCandidateIsFeasible(
    edge,
    points,
    from,
    to,
  ...
  function properAxisCrossing(a, b, c, d) {
    ...
    return (
      x > Math.min(horizontal[0][0], horizontal[1][0]) + 0.0001 &&
    ...
    for (const [otherEdge, routed] of pathCache) {
      if (
        [edge.from, edge.to].some(
          (id) => id === otherEdge.from || id === otherEdge.to,
        )
      )
        continue;
  ...
  function automaticForwardReversePx(edge, points) {
    ...
    if (
      !from ||
      !to ||
      ["return", "error"].includes(edge.role) ||
      to.col <= from.col
    )
      return 0;
    return points
  ...
  function readableCandidateCost(
    ...
    const directLength =
      Math.abs(points.at(-1)[0] - points[0][0]) +
      Math.abs(points.at(-1)[1] - points[0][1]);
    const interiorPreferred28Deficit = segmentLengths
      .slice(1, -1)
      .reduce((total, length) => total + Math.max(0, 28 - length), 0);
    const xs = points.map(([x]) => x);
    const ys = points.map(([, y]) => y);
    const canvasGrowthPx =
    ...
    const portDisplacementPx =
      Math.abs(points[0][0] - naturalStart[0]) +
      Math.abs(points[0][1] - naturalStart[1]) +
      Math.abs(points.at(-1)[0] - naturalEnd[0]) +
      Math.abs(points.at(-1)[1] - naturalEnd[1]);
    const legacyCoordinateDisplacement =
      Math.abs(from.cx - LEGACY_COLUMN_CENTERS[from.col]) +
    ...
    return {
      ...
      stretchMilli: Math.round(
        (directLength > 0 ? routeLength / directLength : 1) * 1000,
      ),
      canvasGrowthPx,
  ...
  function readableAutomaticCandidateSet(
    ...
    const topY = Math.max(
      8,
      Math.min(laneTop(from.lane), laneTop(to.lane)) - 16,
    );
    const bottomY =
      Math.max(
    ...
    const rawCandidates = [
      ...
      {
        family: "lane-gap-corridor",
        via: corridorViaY(start, end, fromSide, toSide, laneGapY),
      },
      {
        family: "column-gap-corridor",
        via: corridorViaX(start, end, fromSide, toSide, midX),
      },
      {
        family: "outside-left",
        via: corridorViaX(start, end, fromSide, toSide, outsideLeft),
      },
      {
        family: "outside-right",
        via: corridorViaX(start, end, fromSide, toSide, outsideRight),
      },
      {
        family: "top-corridor",
        via: corridorViaY(start, end, fromSide, toSide, topY),
      },
      {
        family: "bottom-corridor",
        via: corridorViaY(start, end, fromSide, toSide, bottomY),
      },
    ];
  ...
  function readableAutomaticVia(edge, from, to, start, end, fromSide, toSide) {
    const { rawCandidates, candidates, outsideRight } =
    ...
    const outsideRightCandidate = rawCandidates.find(
      ({ family }) => family === "outside-right",
    );
    if (outsideRightCandidate) {
      const currentPoints = normalizeRoutePoints([
        start,
        ...outsideRightCandidate.via,
        end,
      ]);
      ...
      for (const [otherEdge, routed] of pathCache) {
        ...
        if (
          labelRect &&
          otherLabel &&
          rectsOverlap(labelRect, otherLabel, -2)
        ) {
          const rightwardLabelDeficit =
            otherLabel.x + otherLabel.width - 2 - labelRect.x;
          if (rightwardLabelDeficit > 0) {
            outsideRightMinX = Math.max(
              outsideRightMinX,
              outsideRight + rightwardLabelDeficit * 2,
        ...
        for (let index = 0; index < routed.points.length - 1; index += 1) {
          ...
          const rightwardLabelDeficit =
            Math.max(segment.start[0], segment.end[0]) + 4 - labelRect.x;
          if (rightwardLabelDeficit > 0) {
            outsideRightMinX = Math.max(
              outsideRightMinX,
              outsideRight + rightwardLabelDeficit * 2,
      ...
      for (const [otherEdge, routed] of pathCache) {
        for (const [x] of routed.points)
          rightmostPlacedX = Math.max(rightmostPlacedX, x);
        const otherLabel = labelRectFor(
          otherEdge,
          workflow.edges.indexOf(otherEdge),
        );
        if (otherLabel) {
          rightmostPlacedX = Math.max(
            rightmostPlacedX,
            otherLabel.x + otherLabel.width,
          );
        }
      }
      ...
      for (let probe = 0; probe < 7; probe += 1) {
        if (outsideRightMinX > outsideRight + 0.0001) {
          ...
          if (
            ...
            for (
              ...
              const midpointX =
                Math.ceil(((infeasibleX + feasibleX) / 2) * 1000) / 1000;
        ...
        outsideRightMinX =
          Math.ceil((outsideRightMinX + probeGrowth) * 1000) / 1000;
        probeGrowth *= 2;
      }
    }
    const hasRelevantAbsolutePin =
      Array.isArray(edge.labelAt) ||
      [...pathCache.keys()].some(
        (otherEdge) =>
          Array.isArray(otherEdge.labelAt) || hasAbsoluteRoutePins(otherEdge),
      );
    if (hasRelevantAbsolutePin)
      classifyFailedAutomaticCandidatePins(edge, rawCandidates);
  ...
  function readablePresetVia(edge, from, to, start, end, fromSide, toSide) {
    ...
    for (const candidatePreset of [
      ...
      if (
        ...
        supportedFixes.push(
          `set edge "${edgeName}" route to verified preset "${candidatePreset}"`,
        );
      }
    }
    if (
      ...
      supportedFixes.push(
        `remove route from edge "${edgeName}" so readable-v2 can use its verified automatic candidate`,
      );
    }
    throwDiagnosticError(message, [
      {
  ...
  function routeVia(
    ...
    switch (edge.route || "auto") {
      ...
      case "drop": {
        const y =
          edge.channelY ?? gapYBetween(from.lane, to.lane, edge.bias ?? 0.5);
        return [
          [start[0], y],
          [end[0], y],
        ];
      ...
      default: {
        if (workflow.schema_version === 2) {
          if (edge.channelX !== undefined && edge.channelY !== undefined) {
            return [
              [edge.channelX, start[1]],
              [edge.channelX, edge.channelY],
              [end[0], edge.channelY],
            ];
          }
          if (edge.channelX !== undefined)
            return [
              [edge.channelX, start[1]],
              [edge.channelX, end[1]],
            ];
          if (edge.channelY !== undefined)
            return [
              [start[0], edge.channelY],
              [end[0], edge.channelY],
            ];
          return readableAutomaticVia(
            edge,
        ...
        const oneBendVia = oneBendCrossLaneVia(
          edge,
          start,
          end,
          fromSide,
  ...
  function workflowEdgeLabelPoint(edge, points) {
    if (workflow.schema_version === 1) {
      if (
        edge.labelAt ||
        Number.isInteger(edge.labelSegment) ||
        points.length !== 3
      ) {
        return labelPoint(edge, points);
      }
      const segmentLengths = [0, 1].map((index) =>
        Math.hypot(
      ...
      if (points[labelSegment][0] === points[labelSegment + 1][0])
        point[1] += 10;
      return point;
    }
    if (
  ...
  function edgeSides(edge) {
    ...
    const to = nodes.get(edge.to);
    const resolved =
      workflow.schema_version === 2 ? readableSideCache.get(edge) : null;
  ...
  const automaticPorts = automaticPortSpread(workflow.edges, nodes, {
    sideFor: (edge, endpoint) =>
      edgeSides(edge)[endpoint === "source" ? "fromSide" : "toSide"],
  });
  ...
  function readableAutomaticRoute(edge, from, to, primarySides, primaryPorts) {
    ...
    for (const candidateSides of sidePairs) {
      ...
      const start =
        primaryPorts?.from && primary
          ? primaryPorts.from
          : anchor(from, candidateSides.fromSide);
      const end =
        primaryPorts?.to && primary
          ? primaryPorts.to
      ...
      try {
        const expandedVia = withDiagnosticRecordingSuppressed(() =>
          readableAutomaticVia(
            edge,
            from,
            to,
            start,
        ...
        const outsideRightOrdinal = planned.rawCandidates.findIndex(
          ({ family }) => family === "outside-right",
        );
    ...
    if (authoredSideFields.length) {
      ...
      const sourceAnchor =
        primaryPorts?.from || anchor(from, primarySides.fromSide);
      const targetAnchor = primaryPorts?.to || anchor(to, primarySides.toSide);
      const attemptedEvidence =
        firstFailure?.archifyDiagnostics?.[0]?.evidence || {};
      throwExplicitPinConflict(
        edge,
    ...
    throw new Error(
      "readable-v2 automatic route enumeration produced no result",
    );
  }
  ...
  function readableControlledRoute(edge, from, to) {
    ...
    for (const [ordinal, { fromSide, toSide }] of sidePairs.entries()) {
      const start = anchor(from, fromSide);
      const end = anchor(to, toSide);
      const via = routeVia(edge, from, to, start, end, fromSide, toSide, {
      ...
      if (
        edge.route &&
        edge.route !== "auto" &&
        !routeMatchesPresetFamily(edge.route, points, from, to)
      )
        continue;
      if (
        presentChannelPins(edge).some(
          (field) => !routeContainsChannelPin(points, field, edge[field]),
        )
      )
        continue;
  ...
  function pathFor(edge) {
    ...
    const authoredPoints = [
      start,
      ...routeVia(edge, from, to, start, end, fromSide, toSide),
      end,
    ];
    const hasAbsoluteRoutePins =
      Array.isArray(edge.via) ||
      ...
      edge.channelY !== undefined;
    const points =
      workflow.schema_version === 2 && !hasAbsoluteRoutePins
        ? normalizeRoutePoints(authoredPoints)
        : authoredPoints;
  ...
  function labelRectFor(edge, relationIndex) {
    if (!edge.label || !nodes.has(edge.from) || !nodes.has(edge.to))
      return null;
  ...
  function measuredContentBounds() {
    ...
    const owners = {
      left: "workflow lanes",
      top: asArray(workflow.phases).length
        ? "phase header band"
        : "workflow top padding",
      right: "workflow lanes",
      bottom: workflowLegendEntries.length
        ? "legend"
        : "workflow lanes and bottom padding",
    };
    ...
    for (const [index, edge] of workflow.edges.entries()) {
      ...
      for (const point of pathFor(edge).points)
        includePoint(point, `edge ${edge.id || index}`);
    ...
    for (const phase of asArray(workflow.phases)) {
      ...
      const span = phaseSpan(phase);
      includeRect(
        { x: span.x, y: 27, width: span.width, height: 16 },
        `phase ${phase.id}`,
      );
    }
    for (const group of asArray(workflow.groups)) {
    ...
    if (workflowLegendEntries.length) {
      for (const rect of workflowLegendRects())
        includeRect(rect, `legend ${rect.kind}`);
    }
    return {
      left,
      top,
  ...
  function finalizeReadableViewBox() {
    ...
    if (outsideOrigin) {
      const hasAbsolutePins = workflow.edges.some(
        (edge) =>
          Array.isArray(edge.via) ||
          Array.isArray(edge.labelAt) ||
          edge.channelX !== undefined ||
      ...
      throwDiagnosticError(message, [
        {
          code: hasAbsolutePins
            ? "workflow/explicit-pin-conflict"
            : "workflow/solver-budget-exhausted",
          severity: "error",
          message,
    ...
    if (
      ...
      supportedFixes.push(
        `set meta.viewBox to at least [${requiredViewBox[0]}, ${requiredViewBox[1]}]`,
      );
    }
    if (
      acceptsFix((document) => {
      ...
      supportedFixes.push(
        "omit meta.viewBox so the compiler can use its measured intrinsic canvas",
      );
    }
    throwDiagnosticError(message, [
      {
  ...
  function renderLane(lane, index) {
    ...
    const prefix =
      lane.variant === "exception" ? "EX" : String(index + 1).padStart(2, "0");
  ...
  function renderPhase(phase) {
    ...
    const [lineClass] =
      arrowClassMap[phase.variant || "default"] || arrowClassMap.default;
  ...
  function renderGroup(group, index) {
    ...
    const labelY =
      workflow.schema_version === 2 ? y + GROUP_LABEL_BASELINE_OFFSET : y + 14;
  ...
  function renderNode(node) {
    ...
    const tag = node.tag
      ...
      : "";
    const brand = renderBrandMark(node, {
...
${renderLegend()}
  ...
  try {
    ...
    const diagnostics = error.archifyDiagnostics.map((diagnostic) => ({
      ...diagnostic,
    }));
...
function compileWorkflowWithFeedback({
  workflow,
  qualityProfile,
  discoverFixes = true,
} = {}) {
  ...
  for (
    let attempt = 0;
    attempt <= MAX_READABLE_LAYOUT_FEEDBACK_ROUNDS;
    attempt += 1
  ) {
    try {
