...
export function planningWorkflow(workflow) {
  ...
  planned.edges = planned.edges.flatMap((edge) => {
    ...
    if (edge.labelAt === undefined && edge.label !== undefined)
      automatic.label = edge.label;
    return [automatic];
  });

  if (Array.isArray(planned.mainPath)) {
    const projectedPairs = new Set(
      planned.edges.map((edge) => `${edge.from}\u0000${edge.to}`),
    );
...
export function createHorizontalRankMapper(oldColumns, newColumns) {
  if (
    ...
    throw new TypeError(
      "Horizontal rank mapping requires matching finite column arrays.",
    );
  }
  for (let index = 1; index < oldColumns.length; index += 1) {
    if (
      oldColumns[index] <= oldColumns[index - 1] ||
      newColumns[index] <= newColumns[index - 1]
    ) {
      throw new TypeError(
        "Horizontal rank mapping requires strictly increasing columns.",
      );
    }
  }
  ...
  return (x) => {
    if (!Number.isFinite(x))
      throw new TypeError(
        "Horizontal rank mapping requires a finite x coordinate.",
      );
 ...
 */
export function createMappedWorkflowCandidate(
  workflow,
  oldColumns,
  newColumns,
) {
