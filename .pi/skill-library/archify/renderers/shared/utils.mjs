...
const SVG_SLOT_RE =
  / {6}<!-- ARCHIFY:SVG_SLOT_START -->[\s\S]*? {6}<!-- ARCHIFY:SVG_SLOT_END -->/;
const CARDS_SLOT_RE =
  / {4}<!-- ARCHIFY:CARDS_SLOT_START -->[\s\S]*? {4}<!-- ARCHIFY:CARDS_SLOT_END -->/;
...
export function applyTemplate(
  ...
  for (const ph of TEMPLATE_PLACEHOLDERS) {
    if (!template.includes(ph)) {
      throw new Error(
        `applyTemplate: template missing placeholder ${JSON.stringify(ph)}`,
      );
    }
  }
