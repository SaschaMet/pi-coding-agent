...
export function loadDiagram({
  ...
  const inputPath = path.resolve(
    argv[2] || path.join(skillRoot, "examples", defaultExample),
  );
  ...
  const template = fs.readFileSync(
    path.join(skillRoot, "assets/template.html"),
    "utf8",
  );
  const outputRequest = {
...
export function writeDiagram({
  ...
  if (!START_TYPES.has(diagramType))
    throw new Error(
      `writeDiagram: unknown diagram type ${JSON.stringify(diagramType)}`,
    );
...
export function validateRelationshipIds(diagramType, diagram) {
  ...
  if (problems.length) {
    throwDiagnosticProblems(
      "Relationship identity validation failed",
      problems,
      {
...
export function validateGuidedViews(diagramType, diagram) {
  ...
  const semanticIds = new Set(
    (diagram[collection] || []).map((item) => item.id),
  );
  const seen = new Set();
  const problems = [];
  ...
  views.forEach((view, index) => {
    if (seen.has(view.id))
      problems.push(
        `/meta/views/${index}/id duplicates view id ${JSON.stringify(view.id)}`,
      );
    seen.add(view.id);
...
export function svgRootAttrs(meta) {
  ...
  const requestedProfile =
    process.env.ARCHIFY_QUALITY_PROFILE || meta.quality_profile;
  const qualityProfile =
    requestedProfile === "showcase" ? "showcase" : "standard";
...
export function svgAccessibleText(meta, kind) {
  const description =
    meta.subtitle ||
    translateMessage(meta.locale, `diagram.description.${kind}`);
...
export function animateAttr(meta, kind, step) {
  ...
  const safeStep =
    Number.isFinite(step) && step >= 0 ? Math.min(12, Math.floor(step)) : 0;
