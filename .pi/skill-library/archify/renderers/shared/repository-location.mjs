...
export function parseRepositoryRemote(value, { authored = false } = {}) {
  ...
  if (protocol === "ssh:" && (url.username !== "git" || url.password))
    return null;
  if (authored && protocol !== "ssh:" && (url.username || url.password))
    return null;
  ...
  if (provider)
    segments[last] = segments[last].replace(
      provider === "github" ? /\.git$/i : /\.git$/,
      "",
    );
  if (!segments[last] || segments[last] === "." || segments[last] === "..")
    return null;
  ...
  const port =
    url.port ||
    (protocol === "ssh:" ? "22" : protocol === "https:" ? "443" : "80");
  const endpoint =
    provider &&
    ((protocol === "https:" && port === "443") ||
      ...
      ? "standard"
      : `${protocol}${port}`;
  const pathKind = provider
    ? "repository"
    : scp && !scpAbsolute
      ? "relative"
      : "absolute";
  const identityPath =
    provider === "github" ? repositoryPath.toLowerCase() : repositoryPath;
  const encodedPath = segments.map(encodeURIComponent).join("/");
  const canonicalUrl = scp
    ? `git@${hostname}:${scpAbsolute ? "/" : ""}${repositoryPath}`
