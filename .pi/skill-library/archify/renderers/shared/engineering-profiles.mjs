...
function membership(boundaries, componentId, kind) {
  return boundaries
    ...
    .filter(
      ({ boundary }) =>
        boundary.kind === kind && boundary.wraps.includes(componentId),
    );
}

export function deploymentOwnershipDiagnostics(diagram) {
  const components = Array.isArray(diagram.components)
    ? diagram.components
    : [];
  const boundaries = (
    Array.isArray(diagram.boundaries) ? diagram.boundaries : []
  ).map((boundary) => ({
    ...boundary,
    wraps: Array.isArray(boundary.wraps) ? boundary.wraps : [],
  }));
  ...
  components.forEach((component, index) => {
    ...
    if (regions.length === 0) {
      ...
      diagnostics.push({
        ...
        evidence: {
          ...
          regions: regions.map(({ boundary, index: boundaryIndex }) => ({
            boundaryIndex,
    ...
    if (PRIVATE_STATE_TYPES.has(component.type)) {
      const privateScopes = membership(
        boundaries,
        component.id,
        "security-group",
      );
  ...
  boundaries.forEach((boundary, index) => {
    ...
    const members = boundary.wraps.map((id) => ({
      id,
      regions: membership(boundaries, id, "region").map(
        ({ boundary: region, index: boundaryIndex }) => ({
          boundaryIndex,
          label: region.label,
        }),
      ),
    ...
    const regionIndexes = new Set(
      members.flatMap((member) =>
        member.regions.map((region) => region.boundaryIndex),
      ),
    );
    const consistent =
      members.length > 0 &&
  ...
  connections.forEach((connection, index) => {
    const crossedBoundaries = boundaries
      ...
      .filter(
        (boundary) =>
          DEPLOYMENT_BOUNDARY_KINDS.has(boundary.kind) &&
          boundary.fromInside !== boundary.toInside,
      );
    if (
      crossedBoundaries.length === 0 ||
    ...
    diagnostics.push({
      ...
      evidence: {
        ...
        to: connection.to,
        crossedBoundaries: crossedBoundaries.map(
          ({ boundaryIndex, kind, label }) => ({ boundaryIndex, kind, label }),
        ),
      },
      supportedFixes: [
