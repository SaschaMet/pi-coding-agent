...
export function relationshipLegendObstacles(
  relations,
  { pointsFor, labelRectFor } = {},
) {
  const obstacles = [];
  for (const [index, relation] of (Array.isArray(relations)
    ? relations
    : []
  ).entries()) {
    const points =
      typeof pointsFor === "function" ? pointsFor(relation, index) : [];
    ...
    for (
      let pointIndex = 0;
      pointIndex < finitePoints.length - 1;
      pointIndex += 1
    ) {
      obstacles.push({
    ...
    const labelRect =
      typeof labelRectFor === "function" ? labelRectFor(relation, index) : null;
    if (
      labelRect &&
      [labelRect.x, labelRect.y, labelRect.width, labelRect.height].every(
        Number.isFinite,
      )
    ) {
...
export function resolveLegend(config, catalog, presentKinds) {
  ...
  const present =
    presentKinds instanceof Set ? presentKinds : new Set(presentKinds || []);
  ...
  return catalog.flatMap((catalogEntry) => {
    ...
    const visible =
      ...
      (selectedByMode && override.visible !== false);
    if (!visible) return [];
    return [
      {
        ...catalogEntry,
        ...
        interactive:
          catalogEntry.interactive !== false && present.has(catalogEntry.kind),
      },
    ];
  });
}
    ...
    throwDiagnosticError(message, [
      {
        ...
        subject: {
          diagramType,
          path: `/meta/legend/entries/${tooWide.kind}/label`,
        },
        evidence: {
          kind: tooWide.kind,
          measuredWidthPx: tooWide.width,
    ...
    obstacles.some((obstacle) =>
      Array.isArray(obstacle.start) && Array.isArray(obstacle.end)
        ? segmentIntersectsRect(
            { start: obstacle.start, end: obstacle.end },
            legendRect,
          )
...
export function renderLegend({ entries, layout, renderSwatch, locale }) {
  ...
  if (!measured) return "";
  const hasInteractiveEntries = measured.entries.some(
    (entry) => entry.interactive,
  );
  const renderedFontSize =
    measured.fontSize < 8 ? measured.fontSize + 0.5 : measured.fontSize + 2;
  const rootAttributes = hasInteractiveEntries
    ? ' data-legend="" data-legend-bridge=""'
    : ' data-legend=""';
  const parts = [
    `        <g${rootAttributes}>`,
