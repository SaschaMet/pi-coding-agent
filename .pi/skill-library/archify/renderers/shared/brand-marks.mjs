...
function validateUrlShape(
  url,
  allowPrivate = process.env.ARCHIFY_BRAND_ALLOW_PRIVATE === "1",
) {
  if (!["https:", "http:"].includes(url.protocol))
    throw new Error("only HTTP(S) brand links are supported");
  if (url.username || url.password)
    throw new Error("brand links cannot contain credentials");
...
function beforeDeadline(promise, deadline) {
  ...
  if (remaining <= 0)
    return Promise.reject(new Error("brand capture timed out"));
  return new Promise((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new Error("brand capture timed out")),
      remaining,
    );
    timer.unref?.();
    promise.then(
...
async function resolveRequestTarget(url, deadline) {
  ...
  if (
    !addresses.length ||
    (!allowPrivate &&
      addresses.some(({ address }) => isPrivateBrandAddress(address)))
  ) {
    throw new Error("private brand links are not fetched");
  }
  return addresses[0];
    ...
    const request = transport.request(
      url,
      {
        ...
        signal: timeoutSignal(
          Math.max(1, Math.min(4500, deadline - Date.now())),
        ),
      ...
      (response) => {
        ...
        resolve({
          ...
          headers: {
            get(name) {
              const value =
                response.headers[String(name).toLocaleLowerCase("en-US")];
...
function iconCandidates(html, pageUrl) {
  ...
  for (const match of html.matchAll(/<link\b[^>]*>/gi)) {
    ...
    try {
      ...
      const score =
        (type.includes("svg") || /\.svg(?:$|[?#])/i.test(url.href)
          ? 1000000
          : 0) +
  ...
  const unique = new Map(
    candidates.map((candidate) => [candidate.url.href, candidate]),
  );
...
async function imageData(response) {
  ...
  const signatureMatches =
    contentType === "image/png"
      ? buffer.length >= 45 &&
        ...
        buffer.toString("ascii", buffer.length - 8, buffer.length - 4) ===
          "IEND"
  ...
  if (!signatureMatches)
    throw new Error(`brand asset bytes do not match ${contentType}`);
  return {
...
async function captureRemoteBrand(
  ...
  try {
    ...
    const html = (await readLimited(page.response, MAX_HTML_BYTES)).toString(
      "utf8",
    );
    const iconErrors = [];
    ...
    const usefulError =
      iconErrors.find((error) =>
        /unsupported brand image type/i.test(error?.message),
      ) || iconErrors.at(-1);
...
export async function captureBrandReference(value) {
  ...
  if (preset)
    return {
      brand: preset.id,
      resolved: { ...preset, kind: "preset", status: "preset" },
    };
  const resolved = await captureRemoteBrand(url.href);
  if (resolved.status !== "captured" || !resolved.sha256) {
    throw new Error(
      `brand capture failed: ${resolved.reason || "no usable site icon was found"}`,
    );
  }
  return {
...
function suggestions(value) {
  ...
  return BRAND_MARKS.map((mark) => ({
    id: mark.id,
    score: lookupForms(mark.id).some(
      (form) => form.includes(needle) || needle.includes(form),
    )
      ? 0
      : 1,
  }))
...
export async function prepareDiagramBrandMarks(diagramType, diagram) {
  ...
  const nodes =
    collection && Array.isArray(diagram[collection]) ? diagram[collection] : [];
  const unknown = [];
