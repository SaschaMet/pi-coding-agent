  ...
  if (!validate(data)) {
    const diagnostics = validate.errors.map((error) => {
      ...
      const subject = {
        diagramType,
        path: annotated.path,
        ...(annotated.identity == null
          ? {}
          : { identity: String(annotated.identity) }),
      };
      const evidence = {
      ...
      const supportedFixes =
        {
          ...
          required: [
            `add required property ${JSON.stringify(error.params?.missingProperty)}`,
          ],
          type: [
            `use ${JSON.stringify(error.params?.type)} at ${annotated.path}`,
          ],
          enum: [
            `choose one of ${JSON.stringify(error.params?.allowedValues || [])}`,
          ],
          pattern: [
            `match the required pattern ${JSON.stringify(error.params?.pattern)}`,
          ],
          minimum: [
            `use a value ${error.params?.comparison || ">="} ${error.params?.limit}`,
          ],
          maximum: [
            `use a value ${error.params?.comparison || "<="} ${error.params?.limit}`,
          ],
