...
import {
  ...
  redactRepositoryRemote,
  repositorySourceHref,
} from "./repository-location.mjs";
...
export function verifyRepositoryEvidence(diagramType, diagram, repoRootInput) {
  ...
  if (!location) {
    evidenceFailure(
      ...
      "/meta/repository/url must be a credential-free HTTP(S) or Git SSH repository address without query, fragment, or dot segments.",
      {
        ...
        supportedFixes: [
          "declare the matching repository address without credentials; use link_mode: local-only for internal repositories",
        ],
      },
    );
  }
  const linkMode = repository.link_mode ?? "web";
  if (!["web", "local-only"].includes(linkMode))
    evidenceFailure(
      "repository-evidence/link-mode-invalid",
      "Repository link_mode must be web or local-only.",
    );
  if (
    repository.provider !== undefined &&
    (!["github", "gitee"].includes(repository.provider) ||
      repository.provider !== location.provider)
  ) {
    evidenceFailure(
  ...
  if (
    ...
    (!location.provider ||
      ...
      location.endpoint !== "standard" ||
      !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(location.path))
  ) {
    evidenceFailure(
  ...
  if (parseRepositoryRemote(origin)?.identity !== location.identity) {
    ...
    evidenceFailure(
      ...
      `Evidence repository origin ${JSON.stringify(safeOrigin)} does not match ${JSON.stringify(repository.url)}.`,
      {
        subject: { repoRoot: realRoot },
        evidence: {
          localOrigin: safeOrigin,
          authoredRepository: repository.url,
        },
        supportedFixes: [
    ...
    repository: {
      ...
      label:
        location.provider === "github"
          ? location.path
          : location.url.replace(/^(?:https?:\/\/|ssh:\/\/git@|git@)/, ""),
      ...(linkMode === "web"
        ? { href: `${location.url}/tree/${revision}` }
        : { linkMode }),
    },
    referenceCount,
    nodes,
