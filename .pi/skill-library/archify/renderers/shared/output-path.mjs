  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    if (stat.isSymbolicLink()) {
      ...
      const linkTarget = path.isAbsolute(link)
        ? link
        : path.resolve(path.dirname(candidate), link);
      return canonicalize(
        path.join(linkTarget, ...segments.slice(index + 1)),
        depth + 1,
      );
    }
...
export function canonicalFuturePath(targetPath) {
  try {
    ...
    throw new OutputPathError(
      `Output path contains a symbolic-link cycle: "${output}".`,
      {
        code: "output/symlink-cycle",
        message:
          "Output path could not be resolved because it contains a symbolic-link cycle.",
        subject: { output },
        evidence: {
          systemCode: "ELOOP",
        ...
        supportedFixes: [
          "remove the symbolic-link cycle or choose an output path outside it",
        ],
      },
    );
  }
  ...
  try {
    ...
    if (authored && lookup) {
      if (sameFileIdentity(authored, lookup)) {
        ...
        try {
          result =
            fs.realpathSync.native(authoredPath) ===
            fs.realpathSync.native(lookupPath);
        } catch {
          result = null;
        }
      }
...
function futurePathsAlias(leftPath, rightPath) {
  ...
  if (
    !leftDirectory ||
    !rightDirectory ||
    !sameDirectory(leftDirectory, rightDirectory)
  ) {
    return false;
  }
...
export function resolveOutputPath({
  ...
  if (
    ...
    throw new OutputPathError("meta.output must resolve to an .html file.", {
      ...
      message:
        "meta.output must resolve to an .html file after symbolic links are followed.",
      subject: { output: rawOutput },
      supportedFixes: [
  ...
  if (source === "meta" && !pathIsInside(cwd, outputPath)) {
    throw new OutputPathError(
      "meta.output must stay inside the current working directory.",
      {
  ...
  for (const inputPath of inputPaths) {
    ...
    throw new OutputPathError(`Output must not replace ${inputDescription}.`, {
      ...
      supportedFixes: [
        "choose an output path that is distinct from every input path",
      ],
    });
  }
  for (const otherOutputPath of otherOutputPaths) {
    ...
    throw new OutputPathError("Output targets must use distinct paths.", {
      ...
      subject: {
        output: outputPath,
        conflictingOutput: path.resolve(otherOutputPath),
      },
  ...
  if (source === "cli") {
    ...
    const authoredMatches =
      path.extname(rawOutput).toLowerCase() === requiredExtension;
    ...
    if (!authoredMatches || !resolvedMatches) {
      ...
      throw new OutputPathError(message, {
        code: authoredMatches
          ? "output/cli-resolved-extension"
