...
export function rectsOverlap(a, b, gap = 0) {
  ...
  if (
    !isFinitePoint(a.x, a.y, a.width, a.height, b.x, b.y, b.width, b.height)
  ) {
    return false;
  }
  return !(
...
export function segmentRectClearance(segment, rect) {
  ...
  if (
    ...
    return null;
  if (!isFinitePoint(...start, ...end, rect.x, rect.y, rect.width, rect.height))
    return null;
  ...
  const routeCandidates = asArray(routedRelations)
    .map((entry, fallbackIndex) => {
      ...
      const points = normalizeRoutePoints(
        entry?.points || relation?.routePoints,
      );
      if (!relation || points.length < 2) return null;
      return {
        relation,
        relationIndex: Number.isInteger(entry?.relationIndex)
          ? entry.relationIndex
          : fallbackIndex,
        points,
      };
    ...
    for (const route of routes) {
      ...
      for (
        ...
        if (!nearest || clearance < nearest.clearance) {
          nearest = {
            clearance,
            intersectionLength: segmentRectIntersectionLength(
              { start, end },
              rect,
            ),
            segmentIndex,
...
function endpointSideIssue(points, endpoint, side) {
  ...
  return {
    ...
    expectedDirection:
      endpoint === "source" ? rule.sourceDirection : rule.targetDirection,
  };
}
...
export function cleanEndpointSideProblems({
  ...
  for (const [relationIndex, relation] of asArray(relations).entries()) {
    ...
    const authoredFromSide =
      relation.fromSide && relation.fromSide !== "auto"
        ? relation.fromSide
        : null;
    const authoredToSide =
      relation.toSide && relation.toSide !== "auto" ? relation.toSide : null;
    const hasAuthoredRouteGeometry = Boolean(
      (relation.route && relation.route !== "auto") ||
        Array.isArray(relation.via),
    );
    const inferredFromSide =
      !hasAuthoredRouteGeometry && typeof fromSideFor === "function"
        ? fromSideFor(relation)
        : null;
    const inferredToSide =
      !hasAuthoredRouteGeometry && typeof toSideFor === "function"
        ? toSideFor(relation)
        : null;
    ...
    for (const issue of checks) {
      ...
      const sideField =
        issue.sideOrigin === "inferred"
          ? `inferred ${authoredField}`
          : authoredField;
...
export function cleanFlowProblems({
  ...
  for (const [relationIndex, relation] of asArray(relations).entries()) {
    if (
      ...
      continue;
    if (!obstacleIds.has(relation.from) || !obstacleIds.has(relation.to))
      continue;
    const points = pathFor(relation)?.points;
    if (!Array.isArray(points) || points.length < 2) continue;
    if (
    ...
    for (const obstacle of obstacleList) {
      ...
      if (
        !isFinitePoint(obstacle.x, obstacle.y, obstacle.width, obstacle.height)
      )
        continue;
      let hitSegment = -1;
      for (
...
export function forwardCollinearAnalysisSegments(points) {
  ...
  for (
    ...
    if (
      previous &&
      segmentsContinueForward(previous.start, previous.end, start, end)
    ) {
      previous.end = end;
...
function segmentsContinueForward(firstStart, firstEnd, secondStart, secondEnd) {
  ...
  if (
    Math.abs(firstEnd[0] - secondStart[0]) > epsilon ||
    Math.abs(firstEnd[1] - secondStart[1]) > epsilon
  )
    return false;
  const firstVector = [
...
function pointLiesOnSegment(point, start, end) {
  ...
  if (Math.abs(crossProduct(start, end, point)) > epsilon * length)
    return false;
  return (
...
export function cleanCrossingProblems({
  ...
  if (qualityProfileForGate(profile, profileIsAuthoritative) !== "showcase")
    return [];
...
export function collectAmbiguousCorridors({
  ...
  for (let leftIndex = 0; leftIndex < routed.length; leftIndex += 1) {
    ...
    for (
      ...
      for (
        let leftSegment = 0;
        leftSegment < left.points.length - 1;
        leftSegment += 1
      ) {
        for (
          let rightSegment = 0;
          rightSegment < right.points.length - 1;
          rightSegment += 1
        ) {
...
export function cleanAmbiguousCorridorProblems({
  ...
  if (qualityProfileForGate(profile, profileIsAuthoritative) !== "showcase")
    return [];
  const routedRelations = collectEligibleRoutedRelations({
    relations,
    endpointIds,
    pathFor,
  });

  return collectAmbiguousCorridors({ routedRelations, minOverlapPx }).map(
    (hit) => {
      ...
      const from = hit.overlapStart
        .map((value) => Math.round(value * 10) / 10)
        .join(", ");
      const to = hit.overlapEnd
        .map((value) => Math.round(value * 10) / 10)
        .join(", ");
      ...
      for (const border of frameBorderSegments(frame)) {
        ...
        hits.push({
          ...
          segmentIndex: Math.min(
            ...overlaps.map((overlap) => overlap.segmentIndex),
          ),
          overlapLength: merged.reduce(
...
export function routeBudgetMetrics({
  ...
  for (const routed of asArray(routedRelations)) {
    ...
    for (let index = 0; index < points.length - 1; index += 1) {
      ...
      routeLength += length;
      minSegmentPx =
        minSegmentPx == null ? length : Math.min(minSegmentPx, length);
      if (position === "interior") {
        minInteriorSegmentPx =
          minInteriorSegmentPx == null
            ? length
            : Math.min(minInteriorSegmentPx, length);
      }
      if (length < segmentPx) {
        shortSegmentCount += 1;
    ...
    if (direct > 0.0001) {
      ...
      maxStretch =
        maxStretch == null ? routeStretch : Math.max(maxStretch, routeStretch);
...
export function cleanRouteRhythmProblems({
  ...
  return collectRouteRhythmIssues({
    routedRelations,
    interiorSegmentPx,
    microSegmentPx,
  }).map((hit) => {
...
export function cleanLabelRouteClearanceProblems({
  ...
  return collectLabelRouteClearance({ labels, routedRelations, threshold }).map(
    (hit) => {
      const describe = (relation, relationIndex) => {
        const relationId = relation?.id ? ` id "${relation.id}"` : "";
...
function collectEligibleRoutedRelations({ relations, endpointIds, pathFor }) {
  return asArray(relations)
    .map((relation, relationIndex) => {
      ...
      if (
        endpointIds &&
        (!endpointIds.has(relation.from) || !endpointIds.has(relation.to))
      )
        return null;
...
export function normalizeRoutePoints(points) {
  ...
  for (const point of deduped) {
    while (
      normalized.length >= 2 &&
      collinearForward(normalized.at(-2), normalized.at(-1), point)
    )
      normalized.pop();
...
function frameBorderSegments(frame) {
  ...
  if (frame.shape === "line") {
    ...
    return isFinitePoint(...start, ...end)
      ? [{ side: "line", start, end }]
      : [];
  }
  if (
...
export function labelPoint(item, points) {
  ...
  const segmentIndex = Math.min(
    points.length - 2,
    Math.max(0, item.labelSegment ?? 1),
  );
  const a = points[segmentIndex];
  const b = points[segmentIndex + 1];
  return [
    (a[0] + b[0]) / 2 + (item.labelDx || 0),
    (a[1] + b[1]) / 2 - 10 + (item.labelDy || 0),
  ];
}
...
export function suggestLabelObstacleFix(
  labelRect,
  lx,
  ly,
  obstacle,
