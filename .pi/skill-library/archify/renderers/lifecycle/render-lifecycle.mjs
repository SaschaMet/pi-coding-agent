...
function measureState(state) {
  ...
  const width =
    state.width ||
    (isPhase ? layout.phaseW : isOutcome ? layout.outcomeW : layout.eventW);
  const height =
    state.height ||
    (isPhase ? layout.phaseH : isOutcome ? layout.outcomeH : layout.eventH);
  const xs = isPhase
    ? layout.phaseXs
    : isOutcome
      ? layout.outcomeXs
...
const states = new Map(
  asArray(lifecycle.states).map((state) => [state.id, measureState(state)]),
);
const laneLabels = new Map(
  asArray(lifecycle.lanes).map((lane) => [lane.id, lane.label]),
);
...
function validateLifecycle() {
  ...
  if (states.size !== asArray(lifecycle.states).length)
    problems.push("State ids must be unique.");
  ...
  if (laneIds.size !== asArray(lifecycle.lanes).length)
  ...
  const labelRects = [];
  for (const [transitionIndex, transition] of asArray(
    ...
    const longestLine = Math.max(
      textUnits(transition.label),
      textUnits(transition.note || ""),
    );
...
function routeVia(transition, from, to, start, end, fromSide, toSide) {
  ...
  switch (transition.route || "auto") {
    ...
    case "bottom-channel": {
      const y =
        transition.channelY ??
        Math.max(from.y + from.height, to.y + to.height) + 34;
      return [
        [start[0], y],
        [end[0], y],
      ];
    ...
    case "right-channel": {
      const x =
        transition.channelX ??
        Math.max(from.x + from.width, to.x + to.width) + 36;
      return [
        [x, start[1]],
        [x, end[1]],
      ];
...
const automaticPorts = automaticPortSpread(lifecycle.transitions, states, {
  sideFor: (transition, endpoint) =>
    transitionSides(transition)[endpoint === "source" ? "fromSide" : "toSide"],
});
...
function bandTitles() {
  ...
  const eventLanes = lanes.filter(
    (lane) => lane.id !== "main" && lane.id !== "terminal",
  );
  return [
...
function renderState(state) {
  ...
  const step = state.step
    ...
    : "";
  const brand = renderBrandMark(state, {
    x: state.x + state.width - 22,
    y: state.y + 6,
  });
  ...
  const passport = {
    ...
    tag: state.tag,
    context:
      laneLabels.get(state.lane) ||
      i18nText(lifecycle.meta.locale, "node.context.lifecycle"),
...
function renderTransitionPath(transition, index) {
  const [cls, marker] =
    arrowClassMap[transition.variant || "default"] || arrowClassMap.default;
  const routed = pathFor(transition);
  const strokeWidth =
    transition.width || (transition.variant === "emphasis" ? 2 : 1.1);
