...
const participantW =
  columnFit === "spread"
    ? Math.max(
        ...
        Math.min(
          190,
          Math.round((viewBox[0] - sideMargin * 2) / participantCount) - 24,
        ),
      )
    : 86;
const colGap =
  columnFit === "spread" && participantCount > 1
    ? Math.max(
        108,
        (viewBox[0] - 40 - sideMargin - participantW) / (participantCount - 1),
      )
    : 108;
...
function validateSequence() {
  ...
  for (const message of asArray(sequence.messages)) {
    if (!participants.has(message.from))
      problems.push(
        `Message "${message.label}" references unknown source "${message.from}".`,
      );
    if (!participants.has(message.to))
      problems.push(
        `Message "${message.label}" references unknown target "${message.to}".`,
      );
    if (typeof message.y !== "number")
      problems.push(`Message "${message.label}" must provide a numeric y.`);
    if (
      message.y < layout.lifelineTop + 18 ||
      message.y > layout.lifelineBottom - 18
    ) {
      problems.push(
  ...
  problems.push(
    ...cleanCrossingProblems({
      ...
      routeHint:
        "separate the message y values; lifeline crossings remain allowed",
    }),
  );
  problems.push(
    ...cleanAmbiguousCorridorProblems({
      ...
      profile: sequence.meta?.quality_profile,
      routeHint:
        "separate the message y values so unrelated messages do not visually merge",
    }),
  );
  problems.push(
    ...cleanBorderRunProblems({
  ...
  for (let i = 0; i < placed.length; i += 1) {
    for (
      let j = i + 1;
      j < placed.length && placed[j].y - placed[i].y < 28;
      j += 1
    ) {
  ...
  for (const segment of asArray(sequence.segments)) {
    ...
    if (labelBox.x + labelBox.width > viewBox[0] - 48) {
      ...
      problems.push(
        `Segment "${segment.label}" label (~${Math.round(labelBox.width)}px) exceeds the segment frame's available width (${availableWidth}px) — shorten the label or increase meta.viewBox[0] to at least ${requiredWidth}.`,
      );
    }
  }
  ...
  for (const activation of asArray(sequence.activations)) {
    if (!participants.has(activation.participant))
      problems.push(
        `Activation references unknown participant "${activation.participant}".`,
      );
  ...
  const lastParticipant = asArray(sequence.participants)[
    asArray(sequence.participants).length - 1
  ];
  if (
    lastParticipant &&
    participants.get(lastParticipant.id).cx + layout.participantW / 2 >
      viewBox[0] - 40
  ) {
...
function renderActivation(activation) {
  ...
  const fill =
    componentFill[activation.type] ||
    componentFill[participant.type] ||
    "c-external";
...
function messageLabel(message, x1, x2) {
  ...
  const labelW =
    box?.width || Math.max(34, textUnits(message.label) * 5.2 + 12);
  const accent =
...
function renderMessage(message, index) {
  ...
  const [cls, marker] =
    arrowClass[message.variant || "default"] || arrowClass.default;
...
function renderLegend() {
  ...
  const entries = resolveLegend(
    sequence.meta?.legend,
    LEGEND_CATALOG,
    presentKinds,
  );
