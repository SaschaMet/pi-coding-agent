...
const LEGEND_CATALOG = [
  ...
  kind,
  label: i18nText(arch.meta.locale, `legend.architecture.${kind}`),
}));

// ---- Measure components from free coordinates --------------------------------
function measureComponent(c) {
  const [x, y] = resolveComponentPos(c, grid);
  const [w, h] = Array.isArray(c.size)
    ? c.size
    : [layout.defaultW, layout.defaultH];
  return { ...c, x, y, width: w, height: h, cx: x + w / 2, cy: y + h / 2 };
}

const components = new Map(
  asArray(arch.components).map((c) => [c.id, measureComponent(c)]),
);
...
function resolvedViewBoxWidth(candidateBoundaries) {
  if (
    Array.isArray(arch.meta?.viewBox) &&
    Number.isFinite(arch.meta.viewBox[0])
  ) {
...
function expandBoundaryForReadableTitle(boundary, minimumFontSize) {
  ...
  const requiredWidth =
    boundaryLabelWidth(boundary.label, minimumFontSize) +
...
function measureBoundaryTitle(boundary, minimumFontSize) {
  const availableWidth = Math.max(
    0,
    boundary.width - layout.boundaryLabelFrameInset * 2,
  );
  ...
  const height = Math.max(
    ...
    Math.ceil(fontSize + 7),
  );
  return {
    x: boundary.x + layout.boundaryLabelFrameInset,
    y: boundary.memberTop - layout.boundaryLabelClearance - height,
...
function layoutBoundaryTitles(rawBoundaries, minimumFontSize) {
  ...
  const ordered = rawBoundaries
    ...
    .sort((left, right) => {
      const areaDelta =
        left.boundary.width * left.boundary.height -
  ...
  for (const entry of ordered) {
    const { index } = entry;
    const boundary = expandBoundaryForReadableTitle(
      entry.boundary,
      minimumFontSize,
    );
    ...
    while (guard < rawBoundaries.length + components.size + 1) {
      guard += 1;
      const blockers = [...placedTitles, ...components.values()].filter(
        (candidate) =>
          horizontalOverlap(title, candidate) && rectsOverlap(title, candidate),
      );
      if (!blockers.length) break;
      title.y = Math.min(
        ...blockers.map(
          (blocker) => blocker.y - layout.boundaryLabelRailGap - title.height,
        ),
      );
    }
...
function componentContext(component) {
  ...
  return scopes.length
    ? scopes.join(" › ")
    : i18nText(arch.meta.locale, "node.context.architecture");
}
...
function validateArchitecture() {
  ...
  const requiresNestedBoundaryMembership =
    arch.meta?.engineering_profile === "deployment-ownership";
  ...
  for (const c of components.values()) {
    ...
    if (
      c.x < 0 ||
      c.y < 0 ||
      c.x + c.width > viewBox[0] ||
      c.y + c.height > viewBox[1]
    ) {
      problems.push(
  ...
  for (const boundary of asArray(arch.boundaries)) {
    for (const id of asArray(boundary.wraps)) {
      if (!components.has(id))
        problems.push(
          `Boundary "${boundary.label}" wraps unknown component "${id}".`,
        );
    }
  }
  ...
  for (let leftIndex = 0; leftIndex < boundaries.length; leftIndex += 1) {
    ...
    for (
      ...
      if (
        enforcesBoundaryTitleComposition &&
        rectsOverlap(left.title, right.title)
      ) {
        problems.push(
      ...
      if (shared.length && !leftNested && !rightNested) {
        ...
        const rightOnly = [...rightMembers].filter(
          (id) => !leftMembers.has(id),
        );
        problems.push(
      ...
      const containmentMatchesMembership =
        (leftNested && rightContainsLeft) || (rightNested && leftContainsRight);
  ...
  for (const b of boundaries) {
    if (
      b.x < 0 ||
      b.y < 0 ||
      b.x + b.width > viewBox[0] ||
      b.y + b.height > viewBox[1]
    ) {
      problems.push(
  ...
  for (const [connectionIndex, conn] of asArray(arch.connections).entries()) {
    if (!conn.label || !components.has(conn.from) || !components.has(conn.to))
      continue;
...
function sideAwareBridgeCandidates(start, end, fromSide, toSide) {
  ...
  return rawCandidates
    ...
    .filter(
      (points) =>
        !collinearBacktrack(points[0], points[1], points[2] || points[1]),
    )
    .filter(
      (points) =>
        !collinearBacktrack(
...
function alignFacingPorts(conn, from, to, start, end, fromSide, toSide, ports) {
  ...
  if (hasExplicitGeometry || (!horizontallyFacing && !verticallyFacing))
    return { start, end };
...
function routeVia(conn, from, to, start, end, fromSide, toSide) {
  ...
  switch (conn.route || "auto") {
    ...
    default: {
      ...
      const rhythmBridge = automaticPortRhythmBridge(
        start,
        end,
        fromSide,
        toSide,
      ...
      const minimumStub = 8;
      const fromVerticalSide =
        start[1] === from.y || start[1] === from.y + from.height;
      ...
      const fromHorizontalSide =
        start[0] === from.x || start[0] === from.x + from.width;
...
function pathFor(conn) {
  ...
  const points = [
    start,
    ...routeVia(conn, from, to, start, end, fromSide, toSide),
    end,
  ];
...
function renderConnectionPath(conn, index) {
  const [cls, marker] =
    arrowClassMap[conn.variant || "default"] || arrowClassMap.default;
  const routed = pathFor(conn);
