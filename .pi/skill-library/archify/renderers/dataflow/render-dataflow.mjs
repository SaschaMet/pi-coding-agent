...
const nodes = new Map(
  asArray(dataflow.nodes).map((node) => [node.id, measureNode(node)]),
);
...
function validateDataflow() {
  const problems = [];
  if (nodes.size !== asArray(dataflow.nodes).length)
  ...
  for (const flow of asArray(dataflow.flows)) {
    if (!nodes.has(flow.from))
      problems.push(
        `Flow "${flow.label || flow.from}" references unknown source "${flow.from}".`,
      );
    if (!nodes.has(flow.to))
      problems.push(
        `Flow "${flow.label || flow.to}" references unknown target "${flow.to}".`,
      );
    if (!flow.label)
      problems.push(
        `Flow "${flow.from}" -> "${flow.to}" must include a short data label.`,
      );
    if (nodes.has(flow.from) && nodes.has(flow.to)) {
      ...
      if (Array.isArray(flow.via)) {
        for (
          let segmentIndex = 0;
          segmentIndex < routed.points.length - 1;
          segmentIndex += 1
        ) {
...
function routeVia(flow, from, to, start, end) {
  ...
  switch (flow.route || "auto") {
    ...
    case "bottom-channel": {
      const y =
        flow.channelY ?? Math.max(from.y + from.height, to.y + to.height) + 26;
      return [
        [start[0], y],
        [end[0], y],
      ];
...
const automaticPorts = automaticPortSpread(dataflow.flows, nodes, {
  sideFor: (flow, endpoint) =>
    flowSides(flow)[endpoint === "source" ? "fromSide" : "toSide"],
});
...
function pathFor(flow) {
  ...
  for (const p of rawPoints) {
    ...
    if (
      !prev ||
      Math.abs(p[0] - prev[0]) > 0.0001 ||
      Math.abs(p[1] - prev[1]) > 0.0001
    ) {
      points.push(p);
    }
  }
...
function renderFlowPath(flow, index) {
  const [cls, marker] =
    arrowClassMap[flow.variant || "default"] || arrowClassMap.default;
  const routed = pathFor(flow);
...
function renderLegend() {
  const presentKinds = new Set(
    asArray(dataflow.flows).map((flow) => flow.variant || "default"),
  );
  if ([...nodes.values()].some((node) => node.type === "database"))
    presentKinds.add("database");
  const entries = resolveLegend(
    dataflow.meta?.legend,
    LEGEND_CATALOG,
    presentKinds,
  );
