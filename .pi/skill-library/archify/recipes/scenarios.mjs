const RAW_RECIPES = [
  ...
  },
  {
    ...
    en: {
      ...
      avoidWhen:
        "The goal is only to show static agent components or exact API message timing.",
      include: [
  ...
  },
  {
    ...
    en: {
      ...
      avoidWhen:
        "Order is unimportant and the audience only needs the stable service topology.",
      include: [
        "callers and callees",
  ...
  RAW_RECIPES.map((recipe) =>
    Object.freeze({
      ...
      signals: Object.freeze(
        recipe.signals.map((signal) => Object.freeze(signal.slice())),
      ),
      en: Object.freeze({
        ...recipe.en,
        include: Object.freeze(recipe.en.include.slice()),
      }),
      zh: Object.freeze({
        ...recipe.zh,
        include: Object.freeze(recipe.zh.include.slice()),
      }),
    }),
  ),
);
...
export function startPromptsFor(recipe, lang = "en") {
  ...
  if (!descriptionPrompt) {
    throw new Error(
      `Scenario recipe ${JSON.stringify(recipe.id)} does not define a ${language} start prompt.`,
    );
  }
  const repositoryPrompt =
    recipe.type === "architecture"
      ? copy.prompt
      : language === "zh"
...
export function recommendScenario(query, options = {}) {
  const lang =
    options.lang === "zh" || options.lang === "en"
      ? options.lang
      : detectGuideLanguage(query);
  const ranked = SCENARIO_RECIPES.map((recipe) =>
    scoreRecipe(recipe, query),
  ).sort(
    (left, right) =>
      right.score - left.score ||
      SCENARIO_RECIPES.indexOf(left.recipe) -
        SCENARIO_RECIPES.indexOf(right.recipe),
  );
  const winner =
    ranked[0].score > 0
      ? ranked[0]
  ...
  const confidence =
    winner.score >= 14 ? "high" : winner.score >= 7 ? "medium" : "low";
  return {
    ok: true,
    mode: "recommendation",
    lang,
    ...
    alternatives: ranked
      .filter(
        (entry) => entry.recipe.id !== winner.recipe.id && entry.score > 0,
      )
      .slice(0, 2)
      .map((entry) => ({
        ...localized(entry.recipe, lang),
        score: entry.score,
      })),
  };
}
