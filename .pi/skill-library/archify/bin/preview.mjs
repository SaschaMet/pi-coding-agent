...
function sourceDigest(inputPath) {
  try {
    ...
    return {
      hash: `unreadable:${error.code || "unknown"}`,
      bytes: null,
      missing: true,
    };
  }
...
function compactMessage(value) {
  let text = String(
    value || "Preview build failed without a diagnostic.",
  ).trim();
  ...
  const stackLine = relevant.findIndex(
    (line, index) => index > 0 && /^\s*at\s/.test(line),
  );
...
export async function startPreview(options) {
  ...
  const stagingDirectory = fs.mkdtempSync(
    path.join(outputDirectory, ".archify-preview-"),
  );
  ...
  function publishFailure(
    ...
    state.failure = {
      ...
      message: redactDiagnostic(diagnostic, [
        ...
        ...(options.repoRoot
          ? [[path.resolve(options.repoRoot), "<repo-root>"]]
          : []),
      ]),
    };
    broadcast();
  ...
  function commitCandidate(candidatePath, receipt, generationHash) {
    ...
    try {
      ...
      if (digest !== receipt?.artifact?.sha256) {
        throw new Error(
          "Verified candidate bytes do not match the delivery receipt.",
        );
      }
  ...
  function beginBuild(digest, epoch) {
    ...
    const candidatePath = path.join(
      stagingDirectory,
      `generation-${state.generation}.html`,
    );
    ...
    if (digest.bytes !== null) {
      try {
        fs.writeFileSync(snapshotPath, digest.bytes, {
          flag: "wx",
          mode: 0o600,
        });
      } catch (error) {
        publishFailure(
          {
            stage: "prepare",
            error: `Could not snapshot the observed input: ${error.message}`,
          },
          "",
          "",
          candidatePath,
    ...
    if (options.repoRoot)
      args.push("--repo-root", path.resolve(options.repoRoot));
    let stdout = "";
    let stderr = "";
