...
async function commandCompare(args) {
  ...
  try {
    ...
    const baseChecks = baseResult.checks.checks.filter(
      (check) => check.ok,
    ).length;
    const headChecks = headResult.checks.checks.filter(
      (check) => check.ok,
    ).length;
    const finalReceipt = {
      ...compareIr,
...
async function commandDeliver(args) {
  ...
  try {
    try {
      fs.writeFileSync(specificationSnapshotPath, specification, {
        flag: "wx",
      });
    } catch (error) {
...
async function commandDoctor(args) {
  ...
  const missingFiles = checks.reduce(
    (count, check) => count + check.missing,
    0,
  );
...
async function commandBrands(args) {
  ...
  const { listBrandMarks } = await import(
    "../renderers/shared/brand-marks.mjs"
  );
