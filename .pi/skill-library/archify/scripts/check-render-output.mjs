...
addCheck("single_svg", svgMatches.length === 1, [
  `found ${svgMatches.length} <svg> block(s)`,
]);
...
if (svgMatches.length === 1) {
  ...
  const desktopReadabilityIssue = collectDesktopReadability(
    svgAttrs,
    beforeLegend,
  );
  ...
  const routeMetrics = routeBudgetMetrics({
    routedRelations: routedRelationships,
  });
  const routeRhythmIssues = collectRouteRhythmIssues({
    routedRelations: routedRelationships,
  });
  const ambiguousCorridors = collectAmbiguousCorridors({
    routedRelations: routedRelationships,
  });
  const relationshipLabels = collectRelationshipLabelMasks(
    beforeLegend,
    arrows,
  );
  ...
  composition = {
    ...
    metrics: {
      ...
      minLabelRouteClearance: labelRouteMeasurements.length
        ? Math.round(
            Math.min(...labelRouteMeasurements.map((hit) => hit.clearance)) *
              10,
          ) / 10
        : null,
  ...
  if (legendStart >= 0) {
    ...
    addCheck(
      ...
      collisions.map(
        (hit) =>
          `${hit.arrow.kind} ${hit.arrow.index} crosses legend ${hit.box.label}`,
      ),
    );
  } else {
...
console.log(
  JSON.stringify({ ok, file: htmlPath, checks, composition }, null, 2),
);
...
function collectArrows(fragment) {
  ...
  for (const tag of fragment.matchAll(/<(path|line)\b[^>]*>/gi)) {
    const raw = tag[0];
    if (!/\bclass="[^"]*\ba-(?:default|emphasis|security|dashed)\b/.test(raw))
      continue;
    ...
    arrows.push({
      ...
      routePoints:
        ...
        (borderSegments.length
          ? [
              borderSegments[0].start,
              ...borderSegments.map((segment) => segment.end),
            ]
          : []),
...
function collectRelationshipLabelMasks(fragment, arrows) {
  ...
  for (const match of fragment.matchAll(
    ...
    const owner =
      arrows.find(
        (arrow) =>
          groupAttrs["data-edge-key"] !== undefined &&
          arrow.key === groupAttrs["data-edge-key"],
      ) ||
      containedOwner ||
      arrows.find(
        (arrow) =>
    ...
    if (!owner.id && groupAttrs["data-edge-id"])
      owner.id = groupAttrs["data-edge-id"];
    if (!owner.label && groupAttrs["data-edge-label"])
      owner.label = groupAttrs["data-edge-label"];
    labels.push({
      relation: owner,
...
function roundedRouteMetrics(metrics) {
  return {
    ...metrics,
    maxStretch:
      metrics.maxStretch == null
        ? null
        : Math.round(metrics.maxStretch * 1000) / 1000,
    minSegmentPx:
      metrics.minSegmentPx == null
        ? null
        : Math.round(metrics.minSegmentPx * 10) / 10,
...
function collectRelationshipCrossings(arrows) {
  const relationships = arrows.filter(
    (arrow) => arrow.from && arrow.to && arrow.segments.length,
  );
  const crossings = [];
  for (let leftIndex = 0; leftIndex < relationships.length; leftIndex += 1) {
    ...
    for (
      ...
      rightIndex < relationships.length;
      rightIndex += 1
    ) {
      const right = relationships[rightIndex];
      if (
        [left.from, left.to].some((id) => id === right.from || id === right.to)
      )
        continue;
      let point = null;
...
function relationshipRecord(arrow) {
  ...
  return {
    ...
    label: arrow.label || "",
    collectionIndex:
      Number.isInteger(stableIndex) && stableIndex >= 0
        ? stableIndex
        : arrow.index - 1,
...
function straightPathSegments(d) {
  const tokens =
    d.match(/[MLHVQZmlhvqz]|[-+]?(?:\d*\.)?\d+(?:e[-+]?\d+)?/g) || [];
  const segments = [];
  let i = 0;
  let command = "";
  let current = [0, 0];
    ...
    switch (command.toUpperCase()) {
      ...
      case "L": {
        ...
        while (i + 1 < tokens.length && !isCommand(tokens[i])) {
          ...
          if (command.toUpperCase() === "L" || !first)
...
function collectDesktopReadability(svgAttrs, fragment) {
  ...
  for (const match of fragment.matchAll(
    ...
    if (!worst || candidate.projectedFontPx < worst.projectedFontPx)
      worst = candidate;
  }
  return worst;
}
