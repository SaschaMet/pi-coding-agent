...
- Public and non-trivial functions, methods, and classes have concise documentation comments (maximum 2-3 sentences) in the language's local convention, such as JSDoc/TSDoc, Python docstrings, or equivalent. Comments must provide context beyond what the code already expresses — never paraphrase the code into prose.
...
5. Implement only the selected standard:
   ...
   - require concise documentation comments (maximum 2-3 sentences) for public and non-trivial functions, methods, and classes; match the repo's existing convention; comments must provide context beyond what the code already expresses — never paraphrase the code into prose; describe purpose, parameters, returns, side effects, or raised errors only when useful
