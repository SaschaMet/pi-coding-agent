...
function getSubagentConfig(cwd: string): SubagentRuntimeConfig {
...
async function runSingleAgent(
	...
	try {
		...
		const exitCode = await new Promise<number>((resolve) => {
			const invocation = getPiInvocation(args, strictLocalRuntime, cwd ?? defaultCwd);
			if (!invocation) {
				currentResult.stderr =
					"Subagent blocked: strict local runtime is enabled and node_modules/.bin/pi was not found.";
				resolve(1);
				return;
			}
...
const AgentScopeSchema = StringEnum(["user", "project", "both"] as const, {
	description: 'Which agent directories to use. Default: "both". Use "project" for project-only agents.',
	default: "both",
});
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		description: [
			...
			'Default agent scope is "both" (project + user agents and configured skills).',
			'Set agentScope if you want project-only or user-only resolution.',
		].join(" "),
		parameters: SubagentParams,

		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			const runtimeConfig = getSubagentConfig(ctx.cwd);
			const maxParallelTasks = runtimeConfig.maxParallelTasks;
			const maxConcurrency = runtimeConfig.maxConcurrency;
			const agentScope: AgentScope = params.agentScope ?? runtimeConfig.defaultAgentScope;
