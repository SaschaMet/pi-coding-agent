...
export function expandHomePath(rawPath: string): string {
   ...
   if (
      home &&
      (trimmed.startsWith("$HOME/") || trimmed.startsWith("${HOME}/"))
   ) {
      const prefixLen = trimmed.startsWith("$HOME/")
         ? "$HOME/".length
         : "${HOME}/".length;
   ...
   while (true) {
      try {
         ...
         return path.normalize(
            path.join(realBase, ...partsToReattach.reverse()),
         );
      } catch {
...
export function resolveInputPath(inputPath: string, cwd: string): string {
   ...
   return resolvePathWithRealAncestor(
      path.isAbsolute(normalizedInputPath)
         ? normalizedInputPath
         : path.join(cwd, normalizedInputPath),
   );
}
...
export function isOutsideWorkingDirectory(
   inputPath: string,
   cwd: string,
): boolean {
   return !isWithinRoot(
      resolveInputPath(inputPath, cwd),
      resolvePathWithRealAncestor(cwd),
   );
}
