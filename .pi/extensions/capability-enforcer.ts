...
export default function capabilityEnforcerExtension(pi: ExtensionAPI): void {
	pi.on("session_start", async (_event, ctx) => {
		...
		const validationErrors = validateCapabilityConfig(capabilityConfig);
		if (validationErrors.length > 0 && ctx.hasUI) {
			ctx.ui.notify(`Capability config issues: ${validationErrors.join("; ")}`, "warning");
		}
