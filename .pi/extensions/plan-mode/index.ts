...
// Tools
const PLAN_MODE_TOOLS = ["read", "bash", "grep", "find", "ls", "ask_questions", "ask", "web_search", "fetch_web_page", "subagent"];
...
function loadPlanModeConfig(cwd: string): PlanModeConfig {
	...
	try {
		const parsed = JSON.parse(fs.readFileSync(configPath, "utf-8")) as { planMode?: PlanModeConfig };
...
Restrictions:
- You can only use: read, bash, grep, find, ls, ask_questions/ask, web_search, fetch_web_page, subagent
